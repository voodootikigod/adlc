...
<div style={{ display: 'grid', gap: '0.25rem' }}>
  <span style={{ color: 'var(--adlc-pass)' }}>**0**: gate passes. The check ran and the change met the bar. What a gate does when it finds no material to examine is that gate's own call, not a suite-wide guarantee of 0 — spec-lint, for one, exits 2 on a spec with no acceptance criteria.</span>
