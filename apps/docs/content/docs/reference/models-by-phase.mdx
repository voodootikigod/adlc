...
description: A dated binding from the router's abstract tiers to concrete provider models, with verification guidance for changing catalogs.
---
...
1. **"Frontier" means the best model you are allowed to run — not the most expensive
   ...
   *design center*: the lifecycle must hit its accuracy targets with
   ...
   special-access tier. Above-ceiling models are optional headroom, never a requirement.
...
The ADLC Codex plugin does not select or pin a Codex model. Verify availability,
context, and pricing in the [official OpenAI model catalog](https://developers.openai.com/api/docs/models)
before binding a router tier.
