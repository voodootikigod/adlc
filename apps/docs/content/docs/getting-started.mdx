...
# node packages/init/bin/adlc-init.mjs --root /absolute/path/to/project --harness cursor
```

After the matching suite release, the same operation is `adlc init` (or
`adlc init --harness cursor`); Codex users can then invoke `$adlc-init`, and
other harnesses expose equivalent initialization commands on their own release
schedules. See [integrations](/docs/integrations). The
