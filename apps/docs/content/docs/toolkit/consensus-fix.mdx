...
the spec is ambiguous, and the tool escalates rather than guessing. A candidate
proposing no changes is a no-op, not a fix, and is discarded before it can ever
become a survivor — it would otherwise apply cleanly and be judged solely by
whether `--test-cmd` happens to pass a second time against the untouched source.
