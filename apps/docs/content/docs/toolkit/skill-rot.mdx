...
counted against you. Skills with zero checkable claims are reported as `[NO-CLAIMS]` and are never stamped under `--write`. Run it weekly in CI to keep the skill corpus honest.
...
| Flag | Description |
| --- | --- |
| `--write` | When every claim in a skill is ok, upsert `last-verified: <YYYY-MM-DD>` into its frontmatter. Skills with stale claims or zero claims are never stamped. |
