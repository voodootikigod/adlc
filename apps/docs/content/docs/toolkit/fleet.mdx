...
| `--model-plane-read bounded` + `--model-plane-read-only` | Operator-local: bounded model-plane reads (worktree + synthetic home + allowlist; single-file binds for executables) with a private tmpfs `/tmp`. |
...
`resume-refused`, `dispatch-refused`) mean a human is needed.
