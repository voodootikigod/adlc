...
| `--record` | On a clean pass, append a `rails-check` proof (with SHA-256 of every rail-matching file) to `.adlc/manifest.jsonl`. A `sanctionedAdditions` array in this entry (and in `--json` output) names any rail path exempted via the CI-plumbing `--sanctioned-add` flag for this run — empty when none were used, never silent. |
