...
| `--floor <n>` | `0.2` | Minimum rail density for cheap-tier assignment; below it a ticket is forced to frontier and flagged as a P3 finding. `n` must be greater than 0 and at most 1 — `0` is rejected (exit 1) because it would disable the gate entirely. |
...
<div style={{ display: 'grid', gap: '0.25rem' }}>
  ...
  <span style={{ color: 'var(--adlc-wish)' }}>**1**: operational error (bad tickets file, JSON parse error, cycle in the DAG, or `--floor` outside `(0, 1]` — `0` is refused, never a silent kill switch).</span>
