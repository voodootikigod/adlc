...
| `--depth-threshold <n>` | `40` | Tool-call count at or past which a session is degraded (inclusive; `HARD_DEPTH`). |
| `--bytes-threshold <n>` | `8388608` | Transcript bytes (8 MiB) at or past which a session is degraded (inclusive). |
...
The library export `DEFAULT_BYTES_THRESHOLD` remains `HARD_BYTES` (256 KiB) by
design, so the two numbers are not expected to agree: the flag default above is
the recalibrated operating threshold.
