...
```sh
adlc ticket <pull|push|sync|doctor> [--write] [--force] [--allow-rail-narrowing] [--limit <n>] [--json]
```
...
| `--limit <n>` | Cap on issues listed from the tracker (default 500; a positive integer). A listing that reaches the cap is refused as possibly truncated, so raise it or narrow `select`. |
