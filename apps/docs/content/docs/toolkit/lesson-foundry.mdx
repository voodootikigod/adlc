...
```sh
adlc lesson-foundry [--ledger <name>] [--min <n>] [--out-dir <path>] [--write] [--gate] [--allow-missing-ledger] [--tolerate-malformed <n>] [--llm] [--prompt-only] [--json]
```
...
<div style={{ display: 'grid', gap: '0.25rem' }}>
  ...
  <span style={{ color: 'var(--adlc-fail)' }}>**2**: gate fails. One or more clusters have no non-empty defense file in `--out-dir`, or ledger contains malformed lines.</span>
</div>
