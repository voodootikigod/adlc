...
migration and its recovery, archive/restore, transaction recovery, and `ticket-prune`.
A repo where no ticket declares a rail — in the active set or the archive — is not a
trust root: authoring there needs no key and records nothing.
