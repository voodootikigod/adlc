...
records nothing. One residual is accepted deliberately: a repo that removed its last
rail *before* this change and never recorded a hook override leaves nothing on disk to
distinguish it from a repo that never used rails. It is historical-only and cannot
grow — every later removal leaves a marker — and closing it is tracked by ticket
