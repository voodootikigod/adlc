...
1. **In-session (enforcing by default).** A thrown denial in
   ...
   existing path is resolved by `stat`. For a path that does not exist yet the
   KEY decides: `filePath`, `file`, `files[]`, `targetFile`, and apply_patch
   envelopes assert a file; `target`, `path`, `targetPath`, and `edits[]`
   entries are ambiguous and resolve to **directory**, so `{path:
   'assets.bundle'}` still hits an `assets.bundle/**` rail. `targetDir` and a
   trailing slash say directory outright. The cost is over-denial on an absent
   file named ambiguously — the safe direction, recoverable by naming the key.
