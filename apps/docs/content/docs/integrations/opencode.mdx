...
wholesale under an active deny-set, but shell targets are not path-inspected —
with a cold deny-set, a shell command reaching those artifacts is caught by the
CI diff gate, not in-session. Read-only tools keep working.
...
`adlc handoff resume`, a signed bypass, or a privileged host repair.
