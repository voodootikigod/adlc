...
Codex is an **enforcing** tier for the context-rot handoff. A third `PreToolUse`
...
gated wholesale under an active deny-set, but shell targets are not
path-inspected — with a cold deny-set, a shell command reaching those artifacts
is caught by the CI diff gate, not in-session.
