...
**Local checkout (from source).** For local development or installing directly from a local `adlc` source checkout, `agy` takes the path directly:
...
<Callout type="warn">
**Do not point `agy` at the npm location.** `agy` resolves its install target as
`plugin@marketplace` *before* deciding whether that target is a filesystem path,
so an `@` anywhere in the argument is read as the separator. Every location npm
gives a scoped package contains one, so `agy plugin install $(npm root -g)/@adlc/antigravity`
fails with `unknown marketplace: adlc/antigravity` and installs nothing.
`adlc-agy install` stages the plugin under an `@`-free path and installs from
there; `agy` copies the contents into `~/.gemini/config/plugins/adlc-antigravity/`,
so the staging directory is discarded afterwards. A source checkout works directly
