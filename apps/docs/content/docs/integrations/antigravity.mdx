...
<Callout type="warn">
**The `@latest` is load-bearing.** `npx @adlc/antigravity` resolves a bare name
against the **current project** first, so a repository shipping a workspace or
dependency named `@adlc/antigravity` would have *its* binary executed instead —
verified reproducible. A version spec forces registry resolution. It pins nothing;
it only refuses local shadowing, the same reason `install.sh` invokes
`plugins@<version>` rather than bare `plugins`.
</Callout>
