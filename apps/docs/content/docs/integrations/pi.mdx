...
- **Native agent tools.** `adlc_prosecute` runs the deterministic P5 review
  ...
  Both fail closed rather than reporting a result they did not obtain: a gate
  whose process was killed (its 120s timeout, or any signal) or that reported
  no exit code is a **tool error**, never a `PASS`, and records no gate-run
  evidence; and a prosecution in which one or more lenses degraded returns the
  verdict **`INCONCLUSIVE`**, never `CLEAN` — no lens result was obtained,
...
With the flag set, pi enforces the context-rot handoff, and it is the only harness
