...
pi runs the extension in-process and hands out no session id, so one is taken
from the `session_start` event when the host supplies it and otherwise minted
per extension instance. That is exactly the lifetime the denier-sticky rule
needs; a new process gets a new id and is held by the previous session's still
open record.
