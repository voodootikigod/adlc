...
export const routerModel = {
  ...
  harnesses: {
    "claude-code": {
      ...
      // Plugin name is "adlc" (plugins/adlc-claude-code/.claude-plugin/plugin.json), so the
      // actual invocable form of a plugin command inside Claude Code is the namespaced
      // "/adlc:adlc-<name>" — see issue #50. Only claude-code needs this scoped form; the
      // other five harnesses have no plugin-namespace convention (see their slots below).
      slots: {
        "MODEL_PROMPT_NOTE": "**Claude is the model.** Every LLM-backed gate supports `--prompt-only`: it\nprints the exact prompt and exits without calling any provider. Inside Claude\nCode you do not need API keys — run the tool with `--prompt-only`, answer the\nprinted prompt yourself, and apply the judgment. Prefer this over wiring keys.",
        "COLDSTART_LLM_NOTE": "in Claude, use `--prompt-only` and answer the\n  printed audit yourself (the bare form needs an API key and exits 1 without one).",
        "P3_NOTE":" and ",
        "CMD_INIT":"/adlc:adlc-init","CMD_TICKET":"/adlc:adlc-ticket","CMD_DISTILL":"/adlc:adlc-distill","CMD_MAINTAIN":"/adlc:adlc-maintain",
        "P1_ROUTE":"/adlc:adlc-spec → /adlc:adlc-approve-spec (parallax · spec-lint · premortem · adversarial-review)",
        "P1_HEADER_ROUTE":" → `/adlc:adlc-spec`",
        "P1_GATE1":"`/adlc:adlc-spec` runs the loop;\n`/adlc:adlc-approve-spec` records Gate 1 with the interrogation summary\n(`adlc-runner run p1` requires and validates the `spec-approval` record).",
      },
      includes: ["prose:intro","prose:where-am-i","prose:adversarial-loop","prose:phases","prose:notes"],
      local: {
        "sentinel": "<!-- ADLC_CC_SENTINEL_PHASE_ROUTER_V1 -->\n",
      },
    ...
    "cursor": {
      ...
      local: {
        ...
        "commands": "## Cursor command palette\n\nThe scaffolder (`/adlc-init`) deploys these commands into `.cursor/commands/` —\nbare `/adlc-<name>` form (Cursor has no plugin namespace):\n\n- `/adlc-init` — bootstrap the `.adlc/` workspace + Cursor wiring.\n- `/adlc-ticket` — P0 triage through the canonical ticket store.\n- `/adlc-spec` · `/adlc-approve-spec` — P1 interrogation + the human approval gate.\n- `/adlc-decompose` — P2 slicing + merge forecast.\n- `/adlc-verify-build` — P4 deterministic build gate (targeted tests + flail check).\n- `/adlc-prosecute` — P5 prosecution: the five sequential lenses → dedupe → verifier → loop-until-dry, plus the deterministic gates (`hollow-test` · `behavior-diff`) and a cross-model `adversarial-review`. Sequential same-context lenses have **weaker independence** than the siblings' fresh-context fan-out.\n- `/adlc-distill` — P7 lesson-foundry + rejection-mining.\n- `/adlc-maintain` — decay-driven maintenance (`skill-rot` · `model-ratchet` · `ticket-prune` · `gate-fuzzing`).\n\n",
        "rail-enforcement": "## Rail enforcement in this harness\n\nThe bundled hooks wire ONE `preToolUse` dispatcher that runs the rails decision\nfirst — denying structured `Write`/`Edit` to frozen rails declared by the active\nticket — and only on rails-allow **and** `ADLC_BUILD_GATE_ENFORCEMENT=1` consults\nthe advisory buildgate; `afterFileEdit` surfaces a post-edit audit + flail notice\n(it cannot block); `beforeShellExecution` emits an advisory shell reminder that\nnever denies. The rails path is **best-effort / advisory** in-session — Cursor's\n`permission: \"deny\"` is not guaranteed. The buildgate is **advisory, default-off,\nwith NO unbypassable backstop**. The **unbypassable** layer (rails only) is the\ncommit-time CI gate (`docs/ci/rails-guard.yml`). See\n[`docs/integrations/cursor.md`](../../../docs/integrations/cursor.md).\n",
      },
