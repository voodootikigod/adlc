...
function fakeRepo(t) {
  const root = tmp(t, 'adlc-mirror-src-');
...
test('syncMirror flattens the plugin into the target root, adds LICENSE, and transforms the README', (t) => {
  const root = fakeRepo(t);
  const target = tmp(t, 'adlc-mirror-dst-');
...
test('syncMirror does NOT propagate a .github/ directory into the mirror (no workflow-injection path)', (t) => {
  const root = fakeRepo(t);
  const plugin = join(root, 'plugins', 'adlc-herdr');
...
test('syncMirror REMOVES files deleted from the plugin (no stale content) but PRESERVES .git', (t) => {
  const root = fakeRepo(t);
  const target = tmp(t, 'adlc-mirror-dst-');
...
test('syncMirror FAILS CLOSED on a non-empty target that is not a git checkout (never wipes a wrong dir)', (t) => {
  const root = fakeRepo(t);
  const target = tmp(t, 'adlc-mirror-dst-');
...
test('syncMirror rejects a NON-EXISTENT target with a clear message (pins the exists-OR-is-directory guard)', (t) => {
  const root = fakeRepo(t);
  const missing = join(tmp(t, 'adlc-mirror-'), 'does-not-exist');
  assert.throws(() => syncMirror({ repoRoot: root, targetDir: missing }), /target is not a directory/);
});

test('syncMirror rejects a target that is a FILE, not a directory (pins the is-directory guard)', (t) => {
  const root = fakeRepo(t);
  const filePath = join(tmp(t, 'adlc-mirror-'), 'a-file');
  writeFileSync(filePath, 'i am a file');
  assert.throws(() => syncMirror({ repoRoot: root, targetDir: filePath }), /target is not a directory/);
});

test('syncMirror REFUSES to sync into the source repo itself even when it is a git checkout (the `node sync.mjs .` footgun)', (t) => {
  const root = fakeRepo(t);
...
test('syncMirror resolves symlinks before the overlap check (a target symlinked to the source is refused, not wiped)', (t) => {
  const root = fakeRepo(t);
  mkdirSync(join(root, '.git')); // make the source look like a valid checkout so only the overlap guard can stop it
  const link = join(tmp(t, 'adlc-mirror-lnk-'), 'target-link');
...
test('syncMirror FAILS CLOSED if the plugin tree contains a symlink (no write-through, no shipped symlink)', (t) => {
  const root = fakeRepo(t);
  const plugin = join(root, 'plugins', 'adlc-herdr');
  const secret = join(tmp(t, 'adlc-mirror-sec-'), 'secret.txt');
...
test('syncMirror FAILS CLOSED if the monorepo LICENSE is a symlink (the license copy is covered by the guard, not shipped)', (t) => {
  const root = fakeRepo(t);
  const outside = join(tmp(t, 'adlc-mirror-sec-'), 'secret.pem');
...
test('syncMirror REFUSES a target that CONTAINS the source repo (overlap either direction)', (t) => {
  const base = tmp(t, 'adlc-mirror-nest-');
...
test('syncMirror throws when the plugin dir is missing (operational error, not a silent no-op)', (t) => {
  const empty = tmp(t, 'adlc-mirror-norepo-');
  const target = tmp(t, 'adlc-mirror-dst-');
