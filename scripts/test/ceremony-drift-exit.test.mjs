 ...
 */
function makeRepo(t, { drift }) {
  const dir = tmp(t, 'adlc-drift-repo-');
...
function runWith(t, ghScript, { repo, env = {} } = {}) {
  const binDir = tmp(t, 'adlc-drift-bin-');
...
test('a failing `gh` exits non-zero (a broken reporter must not look healthy)', (t) => {
  const repo = makeRepo(t, { drift: true });
  const r = runWith(t, 'echo "gh: HTTP 403 Resource not accessible" >&2; exit 1', { repo });
  assert.notEqual(r.status, 0);
});

test('a `gh` returning unparseable JSON exits non-zero', (t) => {
  const repo = makeRepo(t, { drift: true });
  assert.notEqual(runWith(t, 'printf "%s" "not json"', { repo }).status, 0);
});

test('a missing `gh` binary exits non-zero', (t) => {
  const repo = makeRepo(t, { drift: true });
  const empty = tmp(t, 'adlc-drift-nogh-');
  const r = spawnSync(process.execPath, [SCRIPT], {
...
test('drift present with a working `gh` exits 0 and opens a tracker', (t) => {
  const repo = makeRepo(t, { drift: true });
  const r = runWith(t, stubListing('[]'), { repo });
  assert.equal(r.status, 0, r.stderr);
  assert.match(r.stdout, /1 ticket\(s\) awaiting the completion ceremony/);
  assert.match(r.calls, /issue create/);
});

test('DRY_RUN reports drift and exits 0 without touching `gh`', (t) => {
  const repo = makeRepo(t, { drift: true });
  const r = runWith(t, 'exit 1', { repo, env: { DRY_RUN: '1' } });
  assert.equal(r.status, 0, r.stderr);
...
test('no drift with an open tracker closes it (exit 0)', (t) => {
  const repo = makeRepo(t, { drift: false });
  const r = runWith(t, stubListing(marked(BOT)), { repo });
  assert.equal(r.status, 0, r.stderr);
...
test('no drift and no tracker does nothing (exit 0)', (t) => {
  const repo = makeRepo(t, { drift: false });
  const r = runWith(t, stubListing('[]'), { repo });
  assert.equal(r.status, 0, r.stderr);
...
test('an UNLABELED tracker authored by the bot is adopted, relabeled, and updated', (t) => {
  const repo = makeRepo(t, { drift: true });
  const r = runWith(t, stubUnlabeled(marked(BOT)), { repo });
  assert.equal(r.status, 0, r.stderr);
...
test('a malformed active-ticket pointer still exits 0 and suppresses bulk advice', (t) => {
  const repo = makeRepo(t, { drift: true });
...
test('a well-formed pointer is reported as an exclusion, not as an object', (t) => {
  const repo = makeRepo(t, { drift: true });
...
test('a FORGED marker from another author is not adopted', (t) => {
  const repo = makeRepo(t, { drift: true });
  const r = runWith(t, stubUnlabeled(marked('untrusted-user')), { repo });
  assert.equal(r.status, 0, r.stderr);
...
test('two marked issues fail closed rather than guessing which to overwrite', (t) => {
  const repo = makeRepo(t, { drift: true });
  const two =
  ...
  const r = runWith(t, stubUnlabeled(two), { repo });
...
function makeMixedRepo(t) {
  const dir = tmp(t, 'adlc-drift-mixed-');
...
test('the rendered per-ticket command completes only its named ticket, nothing else', async (t) => {
  const repo = makeMixedRepo(t);
...
for (const drift of [true, false]) {
  test(`a saturated sweep with drift=${drift} exits non-zero and mutates nothing`, (t) => {
    const repo = makeRepo(t, { drift });
    const payloadDir = tmp(t, 'adlc-drift-payload-');
    const r = runWith(t, stubSaturatedSweep(payloadDir), { repo });
...
test('an UNSATURATED sweep finding nothing is treated as an exhaustive miss', (t) => {
  const repo = makeRepo(t, { drift: true });
  const r = runWith(t, stubUnlabeled('[]'), { repo });
  assert.equal(r.status, 0, r.stderr);
  assert.match(r.calls, /issue create/);
});
