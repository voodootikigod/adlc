...
export function canonicalizePath(filePath, cwd) {
...
export function isPathBlocked(filePath, ticket, cwd) {
...
export function isPathInScope(filePath, ticket, cwd) {
    ...
    if (!ticket || !ticket.scope || ticket.scope.length === 0)
        return true;
    // Allow framework directories (except configurations blocked above)
    if (canonical.startsWith(".adlc/") ||
...
export function shellHasMutation(text) {
    return (/(^|[\s;&|])(?:>>?|[0-9]>>?|[0-9]>)\s*\S+/.test(text) ||
...
export function collectShellPaths(text) {
    const out = new Set();
...
export default function (pi) {
    ...
    pi.on("before_agent_start", async (_event, _ctx) => {
        ...
        const sanitizeElement = (el) => el.replace(/[\r\n]/g, " ").replace(/=== ADLC/gi, "").trim();
        ...
        try {
            ...
            for (const line of diffText.split(/\r?\n/)) {
                ...
                if (line.startsWith("@@")) {
                    ...
                    if (match)
                        lineCount = parseInt(match[1], 10) - 1;
                    continue;
                }
