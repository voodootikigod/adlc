...
import {
  mkdirSync, writeFileSync, readFileSync, chmodSync, existsSync,
} from 'node:fs';
...
function makeRepo(t, { tickets = [HIGH_RISK_TICKET, NORMAL_TICKET], current = 'T1' } = {}) {
  const root = tmp(t, 'pi-bg-flail-');
...
test('AC1: high-risk ticket + degraded context denies a structured write', async (t) => {
...
test('AC1: the context-handoff deny outranks the build gate above the handoff band', async (t) => {
  const root = makeRepo(t);
...
test('AC1: below both bands allows; normal-risk ticket is not build-gated', async (t) => {
  const root = makeRepo(t);
  const low = await boot(root, 30);
  assert.equal(await low.pi.handlers.tool_call(writeCall('src/a.ts'), low.ctx), undefined);

  const root2 = makeRepo(t, { current: 'T2' });
...
test('AC1: a normal-risk ticket past the handoff band is still stopped by the deny-set', async (t) => {
  const root3 = makeRepo(t, { current: 'T2' });
...
test('AC1: custom threshold via ADLC_BUILD_GATE_CONTEXT_PCT', (t) => {
  const root = makeRepo(t);
  const verdict = checkBuildGate({
    ticket: HIGH_RISK_TICKET,
    usage: { percent: 60 },
...
test('AC2: threshold/overflow compaction degrades even at low percent; manual does not', async (t) => {
  const root = makeRepo(t);
...
test('AC3: bypass allows only when the override is durably recorded', async (t) => {
  const root = makeRepo(t);
...
test('AC3: bypass with an unwritable ledger stays denied', async (t) => {
  let root;
  t.after(() => { if (root) { try { chmodSync(join(root, '.adlc'), 0o755); } catch {} } });
  root = makeRepo(t);
...
test('AC4: third edit of the same file emits exactly one steer + one notify', async (t) => {
  const root = makeRepo(t, { current: 'T2' });
  const { pi, ctx } = await boot(root, 10);
  for (let i = 0; i < 3; i++) {
...
test('AC5: session_start resets degraded flag and churn window', async (t) => {
  const root = makeRepo(t, { current: 'T2' });
  const { pi, ctx } = await boot(root, 10);
...
test('F4 regression: a rails-denied edit never counts toward churn', async (t) => {
  const railTicket = { ...NORMAL_TICKET, id: 'T3', rails: ['test/contracts/**'] };
  const root = makeRepo(t, { tickets: [HIGH_RISK_TICKET, NORMAL_TICKET, railTicket], current: 'T3' });
  const { pi, ctx } = await boot(root, 10);
  for (let i = 0; i < 4; i++) {
...
test('F4 regression: a throwing sendMessage never breaks the tool call', async (t) => {
  const root = makeRepo(t, { current: 'T2' });
  const pi = fakePi();
