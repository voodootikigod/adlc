...
function makeRepo(t, { tickets = [TICKET], current = 'T24' } = {}) {
  const root = tmp(t, 'pi-ww-');
...
test('AC2: session_start with an active ticket sets a widget naming the ticket', async (t) => {
...
test('AC2: a rail deny refreshes the widget with the gate event', async (t) => {
  const root = makeRepo(t);
...
test('AC2: deactivating the ticket (pointer removed + turn_start) clears the widget', async (t) => {
  const root = makeRepo(t);
  const { pi, ctx } = await boot(root);
...
test('AC2: a threshold compaction refreshes the widget with the degraded flag', async (t) => {
  const root = makeRepo(t);
  const { pi, ctx } = await boot(root);
...
test('AC3: inert session (no ticket) never calls setWidget with content', async (t) => {
...
test('AC4: the full deny path runs on a minimal ctx (no setWidget/getContextUsage)', async (t) => {
  const root = makeRepo(t);
  const pi = fakePi();
  createExtension({ env: {} })(pi);
...
test('AC5: a rail deny emits an adlc-gate-notice message in addition to notify', async (t) => {
  const root = makeRepo(t);
  const { pi, ctx } = await boot(root);
  await pi.handlers.tool_call(
    { type: 'tool_call', toolName: 'write', toolCallId: 'c1', input: { path: 'test/contracts/x.test.ts', content: 'x' } },
...
test('AC5: the build-gate deny path emits an adlc-gate-notice', async (t) => {
  const root = makeRepo(t, { tickets: [HIGH_RISK], current: 'T7' });
...
test('AC5: an advisory gate (unvetted tool) does NOT emit a gate-notice', async (t) => {
  const root = makeRepo(t);
  const { pi, ctx } = await boot(root);
...
test('AC3: P7 staleness renders when manifest carries stale lesson-foundry', async (t) => {
  const root = makeRepo(t);
...
test('AC3: manifest read failure degrades silently (no P7 hint, no error)', async (t) => {
  const root = makeRepo(t);
...
test('AC3: P7 hint suppressed when lesson-foundry is fresh', async (t) => {
  const root = makeRepo(t);
