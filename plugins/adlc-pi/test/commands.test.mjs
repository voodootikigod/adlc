...
function makeRepo(t, { tickets = [T1, T2], current = 'T1' } = {}) {
  const root = tmp(t, 'pi-commands-');
...
test('AC2: /adlc-ticket <id> activates immediately — next tool_call gates the new ticket', async (t) => {
...
test('AC2: /adlc-ticket <unknown-id> notifies an error and leaves the pointer unchanged', async (t) => {
  const root = makeRepo(t, { current: 'T1' });
  const { pi } = await boot(root);
  const ctx = fakeCtx(root);
...
test('AC2: /adlc-ticket getArgumentCompletions completes ticket ids by prefix', async (t) => {
  const root = makeRepo(t, { current: 'T1' });
  const { pi } = await boot(root);
...
test('#104: getArgumentCompletions excludes a completed ticket and keeps the open one', async (t) => {
  const root = makeRepo(t, { tickets: [T1, T2_DONE], current: 'T1' });
  const { pi } = await boot(root);
  const all = pi.commands['adlc-ticket'].getArgumentCompletions('');
  assert.deepEqual(all.map((i) => i.value), ['T1'], 'the completed ticket is not offered as a completion');
});

test('#104: the interactive picker offers only open tickets, and activates the right one', async (t) => {
  const root = makeRepo(t, { tickets: [T1, T2_DONE], current: 'T1' });
  const { pi } = await boot(root);
  let offered = null;
...
test('#104: the selection maps back through the FILTERED list, not the raw ticket index', async (t) => {
  ...
  const root = makeRepo(t, { tickets: [{ ...T1, completed: true }, T2], current: 'T2' });
  const { pi } = await boot(root);
  let offered = null;
  const ctx = fakeCtx(root, { select: (_title, options) => { offered = options; return options[0]; } });
  await pi.commands['adlc-ticket'].handler('', ctx);
...
test('#104: a completed ticket is still activatable by explicit id (lookup, not enumeration)', async (t) => {
  const root = makeRepo(t, { tickets: [T1, T2_DONE], current: 'T1' });
  const { pi } = await boot(root);
  const ctx = fakeCtx(root);
  await pi.commands['adlc-ticket'].handler('T2', ctx);
...
test('AC3: /adlc-ticket with no args offers one option per ticket and activates the choice', async (t) => {
  const root = makeRepo(t, { current: 'T1' });
  const { pi } = await boot(root);
  let offered = null;
...
test('AC3: /adlc-ticket select returning undefined (non-TUI cancel) leaves state unchanged and notifies', async (t) => {
  const root = makeRepo(t, { current: 'T1' });
  const { pi } = await boot(root);
...
test('AC3: /adlc-ticket with no args in non-TUI mode requires an id (no hang, no prompt)', async (t) => {
  const root = makeRepo(t, { current: 'T1' });
  const { pi } = await boot(root);
...
test('AC4: /adlc-approve-spec on an existing file with confirm=true appends a chain-valid entry naming the spec', async (t) => {
  const root = makeRepo(t, { current: 'T1' });
  const { pi } = await boot(root);
...
test('AC4: /adlc-approve-spec with confirm=false records nothing', async (t) => {
  const root = makeRepo(t, { current: 'T1' });
  const { pi } = await boot(root);
  const specRel = '.adlc/specs/feature.md';
  mkdirSync(join(root, '.adlc', 'specs'), { recursive: true });
...
test('AC4: /adlc-approve-spec with no prior interrogation evidence refuses before opening a dialog', async (t) => {
  const root = makeRepo(t, { current: 'T1' });
  const { pi } = await boot(root);
  const specRel = '.adlc/specs/feature.md';
  mkdirSync(join(root, '.adlc', 'specs'), { recursive: true });
...
test('AC4: /adlc-approve-spec refuses when spec-lint is the ONLY prior evidence (spec-lint asks zero human questions)', async (t) => {
  const root = makeRepo(t, { current: 'T1' });
  const { pi } = await boot(root);
  const specRel = '.adlc/specs/feature.md';
  mkdirSync(join(root, '.adlc', 'specs'), { recursive: true });
...
test('AC4: /adlc-approve-spec on a nonexistent path errors without opening a dialog', async (t) => {
  const root = makeRepo(t, { current: 'T1' });
  const { pi } = await boot(root);
  const ctx = fakeCtx(root, { confirm: () => true });
...
test('AC4: /adlc-approve-spec with no path errors without a dialog', async (t) => {
  const root = makeRepo(t, { current: 'T1' });
  const { pi } = await boot(root);
  const ctx = fakeCtx(root, { confirm: () => true });
...
test('AC5: /adlc-init on a bare repo creates sharded stores + gitignore entries and is idempotent', async (t) => {
  const root = tmp(t, 'pi-commands-');
  const pi = fakePi();
  createExtension({ env: {} })(pi);
  const ctx = fakeCtx(root);
...
test('P5 follow-up: /adlc-init never clobbers a POPULATED tickets.json (byte-identical after run)', async (t) => {
  const root = tmp(t, 'pi-commands-');
  mkdirSync(join(root, '.adlc'), { recursive: true });
...
test('/adlc-init previews legacy migration and decline preserves the writable flat file', async (t) => {
...
test('/adlc-init migrates legacy storage only after explicit approval', async (t) => {
  const root = makeRepo(t, { current: null });
  const pi = fakePi({ exec: async (_cmd, args) => {
    if (args[0] === '--version') return { code: 0, stdout: '1.3.0' };
...
test('AC5: /adlc-init with the adlc CLI missing notifies the install command and does not scaffold', async (t) => {
  const root = tmp(t, 'pi-commands-');
...
test('AC5: /adlc-init with the adlc CLI returning non-zero also refuses to scaffold', async (t) => {
  const root = tmp(t, 'pi-commands-');
...
test('AC4: the recorded spec-approval satisfies the real runner p1 assertion (producer/consumer round-trip)', async (t) => {
  const root = makeRepo(t, { current: 'T1' });
  const { pi } = await boot(root);
  mkdirSync(join(root, '.adlc', 'specs'), { recursive: true });
...
test('AC4: recorded rounds/questions reflect real entry count, not distinct-source cardinality or a fixed constant', async (t) => {
  const root = makeRepo(t, { current: 'T1' });
  const { pi } = await boot(root);
  mkdirSync(join(root, '.adlc', 'specs'), { recursive: true });
  const specRel = '.adlc/specs/feature.md';
...
test('AC4: /adlc-approve-spec with no active ticket errors without a dialog (p1 requires a ticket-bound approval)', async (t) => {
  const root = makeRepo(t, { current: null });
  const { pi } = await boot(root);
  mkdirSync(join(root, '.adlc', 'specs'), { recursive: true });
  const specRel = '.adlc/specs/feature.md';
...
function segmentedRepo(t, branch = 'feat/pi-segment-fixture') {
  const root = tmp(t, 'pi-commands-');
...
test('AC4: /adlc-approve-spec finds interrogation evidence recorded in a segmented (forest) manifest, not just the legacy root file', async (t) => {
  const { root, branch } = segmentedRepo(t);
