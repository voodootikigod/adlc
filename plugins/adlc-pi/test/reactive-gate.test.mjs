...
function makeRepo(t, { tickets = [TICKET], current = 'T1' } = {}) {
  const root = tmp(t, 'pi-reactive-gate-');
...
test('snapshot/restore round-trip preserves exact pre-tool state', (t) => {
...
test('listRailFiles: trust roots + rail-glob matches from ls-files/status', (t) => {
  const root = makeRepo(t);
...
test('AC1: violating write is reverted to pre-tool state; user edits survive', async (t) => {
  const root = makeRepo(t);
...
test('clean write passes and leaves the file as written', async (t) => {
  const root = makeRepo(t);
  const { pi, ctx } = await boot(root);
...
test('AC2: untracked rail-file creation by an allowed bash mutation is detected and removed', async (t) => {
  const root = makeRepo(t);
...
test('bash rail EDIT is restored to pre-command content (not HEAD)', async (t) => {
  const root = makeRepo(t);
...
test('degraded bash snapshot never DELETES a modified rail — reports unrestorable', async (t) => {
  const root = makeRepo(t);
  const railPath = join(root, 'test', 'contracts', 'auth.test.ts');
  const pi = fakePi({
...
test('suppression-scan cap trip is surfaced as a warning, not silent', async (t) => {
  const root = makeRepo(t);
...
test('read-only bash commands are not snapshotted or scanned', async (t) => {
  const root = makeRepo(t);
  const { pi, ctx } = await boot(root);
...
test('AC4: ticket activated after session start is picked up on the next turn', async (t) => {
...
test('AC4: ticket switch mid-session swaps the enforcement context', async (t) => {
  const t2 = { id: 'T2', title: 'Other', body: '', scope: ['docs/**'], rails: ['src/**'] };
  const root = makeRepo(t, { tickets: [TICKET, t2], current: 'T1' });
  const { pi, ctx } = await boot(root);
