...
function fixture(t, tickets) {
  const root = tmp(t, 'cursor-bg-');
  const stateDir = tmp(t, 'cursor-bg-state-');
...
test('DENY: high-risk ticket + degraded session (depth past threshold) blocks a structured edit', async (t) => {
  const fx = fixture(t, HIGH_RISK);
...
test('ALLOW: high-risk ticket but shallow session (below threshold)', async (t) => {
  const fx = fixture(t, HIGH_RISK);
  const v = await dispatch(editPayload, { root: fx.root, env: env(fx) });
  assert.deepEqual(v, { permission: 'allow' });
});

test('ALLOW: normal-risk ticket is never gated, no matter how deep the session', async (t) => {
  const fx = fixture(t, NORMAL);
  seedCounter(fx, { count: 10_000, updatedAt: Date.now(), sessionId: 'bg-sess' }, 'bg-sess');
  const v = await dispatch(editPayload, { root: fx.root, env: env(fx) });
  assert.equal(v.permission, 'allow');
});

test('ALLOW: read-only tools are never buildgate-denied (depth still accrues)', async (t) => {
  const fx = fixture(t, HIGH_RISK);
  seedCounter(fx, { count: DEFAULT_DEPTH_THRESHOLD + 5, updatedAt: Date.now(), sessionId: 'bg-sess' }, 'bg-sess');
...
test('DENY: a NOVEL structured mutator name (classifier "other", not shell) is gated like the rails path treats it', async (t) => {
...
test('ALLOW: a shell/terminal tool is NOT buildgate-gated (shell is exempt in-session; writes go to CI)', async (t) => {
  const fx = fixture(t, HIGH_RISK);
  seedCounter(fx, { count: DEFAULT_DEPTH_THRESHOLD + 5, updatedAt: Date.now(), sessionId: 'bg-sess' }, 'bg-sess');
...
test('PATHLESS mutator, NO explicit root → the resolveOwning(payload, undefined) branch does not crash and still gates', async (t) => {
...
test('BYPASS-AUDITED: ADLC_BUILD_GATE_BYPASS=1 allows AND durably records a build-gate-bypass manifest entry', async (t) => {
  const fx = fixture(t, HIGH_RISK);
  seedCounter(fx, { count: DEFAULT_DEPTH_THRESHOLD + 5, updatedAt: Date.now(), sessionId: 'bg-sess' }, 'bg-sess');
...
test('BYPASS-UNAUDITED: an override that cannot be durably recorded is REFUSED (deny)', async (t) => {
  const fx = fixture(t, HIGH_RISK);
  seedCounter(fx, { count: DEFAULT_DEPTH_THRESHOLD + 5, updatedAt: Date.now(), sessionId: 'bg-sess' }, 'bg-sess');
...
test('FAIL CLOSED: conflicting active-ticket signal denies a structured edit under gate enforcement', async (t) => {
  const fx = fixture(t, HIGH_RISK);
...
test('NO-OP: without an active ticket the gate allows (opt-in convention)', async (t) => {
  const fx = fixture(t, HIGH_RISK);
  seedCounter(fx, { count: 10_000, updatedAt: Date.now(), sessionId: 'bg-sess' }, 'bg-sess');
...
test('FRESH SESSION: a stale counter (older than SESSION_TTL_MS) resets — a new session is not denied by stale depth', async (t) => {
  const fx = fixture(t, HIGH_RISK);
...
test('FRESH SESSION: a different session_id uses an independent counter (starts un-degraded)', async (t) => {
  const fx = fixture(t, HIGH_RISK);
...
test('SAME SESSION: a fresh (in-TTL) counter keeps accruing and still denies', async (t) => {
  const fx = fixture(t, HIGH_RISK);
...
test('bumpDepthCounter unit: corrupt counter file counts as a fresh session', (t) => {
  const fx = fixture(t, HIGH_RISK);
...
test('ADLC_BUILD_GATE_DEPTH env input overrides the persisted counter', async (t) => {
  const fx = fixture(t, HIGH_RISK);
...
test('wire format: the real dispatcher script denies under gate enforcement in a degraded session', (t) => {
  const fx = fixture(t, HIGH_RISK);
  seedCounter(fx, { count: DEFAULT_DEPTH_THRESHOLD + 5, updatedAt: Date.now(), sessionId: 'bg-sess' }, 'bg-sess');
...
test('wire format: the real dispatcher script allows the same edit in a fresh session', (t) => {
  const fx = fixture(t, HIGH_RISK);
  const out = execFileSync(process.execPath, [DISPATCHER_SCRIPT], {
    cwd: fx.root,
    env: { ...process.env, ADLC_TICKET: 'T9', ADLC_BUILD_GATE_ENFORCEMENT: '1', ADLC_P4_ENFORCEMENT: '', ADLC_CURSOR_STATE_DIR: fx.stateDir },
