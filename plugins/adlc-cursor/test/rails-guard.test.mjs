...
function fixture(t, { tickets = null, currentTicket = undefined } = {}) {
  const root = tmp(t, 'adlc-cursor-');
  if (tickets) {
...
test('(e) deny an edit to .adlc/tickets.json (trust-root rail)', (t) => {
  const root = fixture(t, { tickets: RAILED });
  const v = decide(payload('Edit', '.adlc/tickets.json'), { root, env: env() });
  assert.equal(v.permission, 'deny');
});

test('(f) a DIFFERENT ticket rail does not deny under the active ticket', (t) => {
  const root = fixture(t, { tickets: RAILED });
...
test('(g) conflicting ADLC_TICKET vs current-ticket.json denies (fail closed)', (t) => {
  const root = fixture(t, { tickets: RAILED, currentTicket: 'T2' });
...
test('(h) afterFileEdit audit NEVER blocks, even on a rail path', (t) => {
  const root = fixture(t, { tickets: RAILED });
...
test('read-only tools are never gated', (t) => {
  const root = fixture(t, { tickets: RAILED });
...
test('(d2) search_replace / str_replace edits to a rail are denied (not waved through as a read)', (t) => {
  const root = fixture(t, { tickets: RAILED });
...
test('(d3) an UNRECOGNIZED structured tool carrying a rail path fails closed (denied)', (t) => {
  const root = fixture(t, { tickets: RAILED });
...
test('(d4) a symlink whose real target is a frozen rail is denied', (t) => {
  const root = fixture(t, { tickets: RAILED });
...
test('(matcher-e2e) a novel mutator name routed to the guard fails CLOSED on a rail', (t) => {
  const root = fixture(t, { tickets: RAILED });
...
test('(F2) corrupt tickets.json fails CLOSED — invalid JSON, throwing schema, and missing tickets', (t) => {
...
test('(multi-root) the guard checks the workspace root that OWNS the edited path', (t) => {
  // repo-a (first root, no ADLC) + repo-b (ADLC-initialized, has the rail).
  const repoA = tmp(t, 'adlc-a-');
  const repoB = fixture(t, { tickets: RAILED });
...
test('(batch) MultiEdit/batch payloads with nested edits[]/files[] are rail-checked, not waved through', (t) => {
  const root = fixture(t, { tickets: RAILED });
...
test('(rename) a rename/move with a frozen rail in EITHER slot (path or target_path) is denied', (t) => {
  const root = fixture(t, { tickets: RAILED });
...
test('(patch) apply_patch command string naming a rail is denied; a non-rail patch allows', (t) => {
  const root = fixture(t, { tickets: RAILED });
...
test('(no-path) a MUTATING tool with no extractable path fails CLOSED under enforcement, OPEN otherwise; reads allow', (t) => {
  const root = fixture(t, { tickets: RAILED });
...
test('(#816) todo_write / create_diagram / glob_file_search / fetch_pull_request allow with no path under enforcement', (t) => {
  const root = fixture(t, { tickets: RAILED });
...
test('(#816 table) the pinned Cursor tool inventory classifies correctly; a novel mutating-shaped name still fails closed', (t) => {
  const root = fixture(t, { tickets: RAILED });
...
test('(no-path multi-root) a pathless opaque tool fails CLOSED if ANY workspace root is actively enforcing', (t) => {
  ...
  const bare = tmp(t, 'adlc-bare-');
  const active = fixture(t, { tickets: RAILED }); // initialized + has a rail
  ...
  const bare2 = tmp(t, 'adlc-bare2-');
  assert.equal(decide({ ...op, workspace_roots: [bare, bare2] }, { env: env() }).permission, 'allow', 'all-inactive must allow');
});

test('(no-path multi-root) shell/read-only tools stay exempt even when a candidate root is conflicted/corrupt', (t) => {
  ...
  const bare = tmp(t, 'adlc-bare-');
  const conflicted = fixture(t, { tickets: RAILED, currentTicket: 'T2' }); // ADLC_TICKET=T1 vs file T2 -> conflict
...
test('(no-path preconditions) a pathless opaque tool NO-OPs when uninitialized / no active ticket', (t) => {
  const op = { tool_name: 'edit_file', tool_input: {} };
  // uninitialized repo (enforcement on, no tickets.json) -> allow (no-op)
  const bare = tmp(t, 'adlc-cursor-');
  ...
  const r = fixture(t, { tickets: RAILED });
...
test('(shell) shell/terminal commands with no file path are NOT denied under enforcement (npm test must run)', (t) => {
  const root = fixture(t, { tickets: RAILED });
  for (const tool of ['Bash', 'run_terminal_cmd', 'terminal', 'shell', 'powershell']) {
    assert.equal(decide({ tool_name: tool, tool_input: { command: 'npm test && npm run build' } }, { root, env: env() }).permission, 'allow', `${tool} must run during P4`);
  }
...
test('(shell-masquerade) a structured mutator named like a shell tool still fails CLOSED', (t) => {
  ...
  for (const tool of ['terminal_edit', 'shell_edit', 'powershell_write', 'shell_replace', 'terminal_modify', 'shell_modify', 'terminal_set', 'powershell_morph']) {
    assert.equal(decide({ tool_name: tool, tool_input: {} }, { root, env: env() }).permission, 'deny', `${tool} must not masquerade as shell`);
  }
  // real shell tools still allowed (regression guard for over-correction)
  for (const tool of ['Bash', 'run_terminal_cmd', 'terminal', 'shell', 'pwsh', 'run_command']) {
    assert.equal(decide({ tool_name: tool, tool_input: { command: 'npm test' } }, { root, env: env() }).permission, 'allow', `${tool} must run during P4`);
  }
});
...
test('(F2-rails) a malformed rail entry (non-string) fails CLOSED, not open', (t) => {
...
test('(multi-root symlink) a NEW file under a symlinked root, in a frozen glob, is denied', (t) => {
...
test('(multi-root relative) a RELATIVE ../ traversal into a DIFFERENT-DEPTH sibling root rail is denied', (t) => {
  ...
  const ws = tmp(t, 'adlc-ws-');
  const app = join(ws, 'app');             // roots[0], uninitialized, depth 1
  const libs = join(ws, 'shared', 'libs'); // active rail, depth 2, different name
...
test('(malformed payload) the hook script fails CLOSED on unparseable JSON under enforcement, OPEN otherwise', (t) => {
  const root = fixture(t, { tickets: RAILED });
  const run = (env) => {
...
test('(multi-root ..) a non-normalized path is attributed to the repo it RESOLVES into', (t) => {
  ...
  const repoA = fixture(t, { tickets: RAILED });
  const repoB = tmp(t, 'adlc-b-');
...
test('wire format: the real script reads stdin JSON and writes a {permission} verdict', (t) => {
  const root = fixture(t, { tickets: RAILED });
