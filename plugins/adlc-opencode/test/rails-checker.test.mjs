...
import { mkdirSync, readdirSync, readFileSync, writeFileSync, symlinkSync } from 'node:fs';
...
test('a: no tickets.json → allow (no-op)', (t) => {
  const dir = tmp(t, 'oc-rails-');
...
test('b: enforcement off → allow even on a declared rail', (t) => {
...
test('c: no active ticket → allow', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
...
for (const tool of ['edit', 'write', 'patch', 'multiedit', 'apply_patch', 'some_new_tool']) {
  test(`d: ${tool} to a declared rail → deny (gated)`, (t) => {
    const dir = repo(t, { tickets: T1_RAILED });
...
test('d: a known read-only tool on a rail path → allow (not a mutation)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
...
test('e: edit .adlc/tickets.json (trust root) → deny even when rails do not list it', (t) => {
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['src/**'] }] } });
...
test('f: another ticket\'s rail does not block (single-active-ticket scope)', (t) => {
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['a/**'] }, { id: 'T2', rails: ['b/**'] }] } });
...
test('e2: edit via a symlink whose real target is a frozen rail → deny', (t) => {
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['src/**'] }] } });
...
test('e2: write through a symlinked parent dir into a frozen rail → deny', (t) => {
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['locked/**'] }] } });
...
test('g: conflicting ADLC_TICKET vs current-ticket.json → deny (fail closed)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED, current: { id: 'T2' } });
...
test('i: mutating tool with NO extractable path while rails in force → deny', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
...
test('i: synthetic third-party write tool (unknown name, unknown args) → deny', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
...
test('i: unextractable path but rails NOT in force → allow (no false deny outside ADLC)', (t) => {
  const dir = tmp(t, 'oc-rails-');
...
test('i: multi-file arg shapes are all checked (edits[] / files[])', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
...
test('j: capitalized tool names are normalized (Read allowed, Edit gated)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('m: bash mutation targeting a frozen rail → deny', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
...
test('m: bash mutation with literal non-rail target → allow', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const r = checkToolCall({ tool: 'bash', args: { command: 'echo x > src/ok.txt' }, root: dir, env: { ...ON, ADLC_TICKET: 'T1' } });
  assert.equal(r.decision, 'allow');
});

test('m: positively read-only bash → allow; with output-option smuggle → deny', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('m: opaque / cwd-changing / expanding / pathless mutations → deny (codex ladder)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('n: rm/mv of a glob rail parent dir → deny (test/** parent is test)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED }); // rails: ['test/**']
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('n: structured single-file write is not ancestor-over-blocked (src/index.mjs vs src/**/test/*.mjs)', (t) => {
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['src/**/test/*.mjs'] }] } });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('n: slash-spelling rails still match direct hits (./test/**, test/, /test/**)', (t) => {
  for (const rail of ['./test/**', 'test/', '/test/**']) {
    const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: [rail] }] } });
...
test('n: a SIBLING dir sharing a name prefix is NOT over-blocked (test2 vs test/**)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
...
test('n: INTERIOR-glob rail ancestor is denied (packages/*/test/** parent packages/foo/test)', (t) => {
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['packages/*/test/**'] }] } });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('n: LEADING-** rail does not over-block unrelated file edits (**/*.test.mjs)', (t) => {
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['**/*.test.mjs'] }] } });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('n: floating leading-** rail — direct hit denied; bare-parent shell delete is CI-gated (boundary)', (t) => {
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['**/*.test.mjs'] }] } });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('n: ANCHORED ** still covers its subtree (src/**/test/*.mjs)', (t) => {
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['src/**/test/*.mjs'] }] } });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('n: partial-segment glob does not over-block a same-depth sibling file (test/*.mjs vs test/x.txt)', (t) => {
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['test/*.mjs'] }] } });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('m: P5 — read-only prefix cannot shadow a rail-targeting writer (git status && curl -o rail)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('m: bash unverifiable command but enforcement OFF or no ticket → allow (no false denies)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
...
for (const tool of ['todowrite', 'question']) {
  test(`k: ungated "${tool}" carrying a frozen-rail filePath → deny (spoof guard)`, (t) => {
    const dir = repo(t, { tickets: T1_RAILED });
...
test('k: ungated tool with a NON-rail target stays allowed', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const r = checkToolCall({ tool: 'todowrite', args: { filePath: 'src/ok.mjs' }, root: dir, env: { ...ON, ADLC_TICKET: 'T1' } });
  assert.equal(r.decision, 'allow');
});

test('k: ungated tool with rail target but enforcement OFF → allow (no false denies)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
...
test('l: unknown no-target tool is denied by default, allowed when operator-ungated', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
...
test('p: adlc_gate nested arg resolving to a frozen rail → deny', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('p: adlc_gate --flag=value payload resolving to a rail → deny', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
  const r = checkToolCall({
    tool: 'adlc_gate',
...
test('p: adlc_gate with benign nested args (flags, non-rail tokens) → allow', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
  const r = checkToolCall({
    tool: 'adlc_gate',
...
test('q: hollow-test via adlc_gate under rails → deny (derives write targets from its --rails file)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('q: unknown/future gate via adlc_gate under rails → deny (fail closed)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('q: gate-manifest DEFAULT dir (.adlc) railed → deny even with no path token in argv', (t) => {
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['.adlc/**'] }] } });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('q: gate-manifest default dir NOT railed → allow (mid-build evidence recording stays legal)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED }); // rails: test/**
  const env = { ...ON, ADLC_TICKET: 'T1' };
  const r = checkToolCall({ tool: 'adlc_gate', args: { gate: 'gate-manifest', args: ['record', 'preflight'] }, root: dir, env });
  assert.equal(r.decision, 'allow');
...
test('q: mutation opt-in flag on an allowlisted gate → deny (--write derives its targets)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('q: comma-separated list token hiding a rail path → deny', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('q: read-only gates still flow under rails; everything unrestricted with rails OFF', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('r: review-calibration via adlc_gate under rails → deny (mutate/restore writer, same class as hollow-test)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
  const r = checkToolCall({
    tool: 'adlc_gate',
...
test('r: --record-verdict writes the manifest ledger — denied when the ledger is railed, allowed otherwise', (t) => {
  const railedLedger = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['.adlc/**'] }] } });
  const normalRails = repo(t, { tickets: T1_RAILED }); // rails: test/**
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('r: model-ratchet --review-cmd is an ARBITRARY PROGRAM — denied under rails; plain dry-run allowed', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('r: command-executor flags are denied for EVERY gate under rails (class, not instance)', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('r: preflight scratch probes are vetted — denied only when a probe path is railed', (t) => {
  const railedAdlc = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['.adlc/**'] }] } });
  const railedGit = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['.git/**'] }] } }); // round-5: branch/worktree metadata churn
  const normalRails = repo(t, { tickets: T1_RAILED }); // rails: test/**
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('r: an ungated tool naming a frozen rail via target is denied', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
...
test('r: an ungated tool naming a non-rail target still runs', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  for (const args of [
    { target: 'src/ok.mjs' },
...
test('r: aliased args cannot slip an ungated tool past an active rail', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
...
test('r: an ABSENT path keeps ancestor detection on the ungated branch, whatever key named it', (t) => {
...
test('r: #498 — the v1.10.0 deny is restored for an absent dotted path, the target/targetDir denies are kept', (t) => {
...
test('r: namesAFile prefers the filesystem and falls back to the name', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
  mkdirSync(join(dir, 'src'), { recursive: true });
  writeFileSync(join(dir, 'src', 'index.mjs'), '');
...
test('r: an existing dotted-name DIRECTORY is not a file, by either spelling', (t) => {
...
test('r: a dot-directory holding a rail keeps ancestor detection', (t) => {
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['.adlc/handoffs/**'] }] } });
...
test('r: a directory-named key keeps ancestor detection even when it looks dotted', (t) => {
  ...
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['assets.bundle/**'] }] } });
  for (const args of [
...
test('r: a generic path key does not assert file-ness, so a rail parent still hits', (t) => {
...
test('r: a claimed file is still denied when the rail NAMES it past a zero-width **', (t) => {
  ...
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['src/**/cache.bundle/**'] }] } });
  const env = { ...ON, ADLC_TICKET: 'T1' };
  for (const args of [
...
test('r: a padded spelling is classified on the same paths it is matched on', (t) => {
...
test('r: narrowed ancestor matching stays linear on a hostile target path', (t) => {
  ...
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['a/**/**/z/**'] }] } });
...
test('r: an embedded-globstar rail still denies its ancestor directory', (t) => {
  ...
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: ['a/foo**bar/**'] }] } });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('r: a rail thousands of globstars deep still yields a decision', (t) => {
  ...
  const dir = repo(t, { tickets: { tickets: [{ id: 'T1', rails: [deep] }] } });
  const env = { ...ON, ADLC_TICKET: 'T1' };
...
test('r: a file-specific key does not make an absent extensionless path a file', (t) => {
...
test('r: conflicted ticket state denies ungated tools too, not just mutators', (t) => {
...
test('r: under conflict a gate must be non-writing, not merely rails-safe', (t) => {
...
test('r: a mutating/unknown tool with only a target still fails closed', (t) => {
  const dir = repo(t, { tickets: T1_RAILED });
