...
test('a blank sessionID reaches the gate as null, not as an id', (t) => {
  const dir = repo(t);
  const seen = [];
  const evaluate = (o) => {
...
test('outside an ADLC repo the gate never evaluates and never denies', (t) => {
...
test('the containment guard uses the caller root, not the process cwd', (t) => {
  ...
  t.after(() => process.chdir(priorCwd));
  const inside = repo(t);
  const outside = plainDir(t);
...
test('the active ticket is threaded to the gate, and resolved only when it is needed', (t) => {
  const dir = repo(t);
  const seen = [];
  let resolutions = 0;
  const evaluate = (o) => {
...
test('a directory contaminated by the OLD bug does not activate enforcement after upgrade', async (t) => {
...
test('an external-store repo is ARMED end to end: the band denies and writes a marker', async (t) => {
...
test('deleting the ticket store mid-session cannot disarm the deny-set', async (t) => {
...
test('deleting the store on the VERY FIRST call still cannot disarm the deny-set', async (t) => {
...
test('the latch only ever arms on a real store, so a contaminated dir stays inert', (t) => {
  const dir = plainDir(t);
  mkdirSync(join(dir, '.adlc', 'handoffs', 'denies'), { recursive: true });
  writeFileSync(join(dir, '.adlc', '.deny-store'), '');
...
test('an initialized repo is still enforced — the guard is not just "no .adlc"', (t) => {
...
test('a failed marker write stays sticky for the session across calls', (t) => {
  const dir = repo(t);
...
test('a custom tool naming a trust-root artifact via target is denied', async (t) => {
...
test('a directory that never opted into ADLC is inert for a whole long session', async (t) => {
...
test('the deny marker is bound to the active ticket, so host repair/resume can act on it', async (t) => {
...
test('a conflicting active-ticket signal binds the marker to nothing, never to a guess', async (t) => {
...
test('the deny message carries the session id and a recovery command against the deny OWNER', async (t) => {
...
test('D2 alone (consumed self-record, no open record left): the message correctly recommends a fresh session as something that actually works', async (t) => {
  const dir = repo(t);
