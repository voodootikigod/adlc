 ...
 * @returns {{ verdict: 'deny'|'allow', out: string, manifest: string }}
 */
function runRails(ticketsJson, relPath, options = {}, t = null) {
  let opts = options;
  let ctx = t;
  if (options && typeof options?.after === 'function' && t === null) {
    ctx = options;
    opts = {};
  }
...
test('no tickets file → allow', (t) => {
  assert.equal(runRails(null, 'src/app.mjs', {}, t).verdict, 'allow');
});

test('schema-valid empty tickets → allow', (t) => {
  assert.equal(runRails('{"tickets":[]}', 'src/app.mjs', {}, t).verdict, 'allow');
});

test('rails declared but path is not a rail → allow', (t) => {
  const tickets = '{"tickets":[{"id":"T1","rails":["test/**"]}]}';
  assert.equal(runRails(tickets, 'src/app.mjs', {}, t).verdict, 'allow');
});

test('sharded ticket store enforces declared rails and freezes every shard', (t) => {
  const dir = tmp(t, 'adlc-rails-sharded-');
...
test('edit to a glob rail → deny', (t) => {
  const tickets = '{"tickets":[{"id":"T1","rails":["test/auth/**"]}]}';
  assert.equal(runRails(tickets, 'test/auth/login.test.mjs', {}, t).verdict, 'deny');
});

test('edit to an exact-file rail → deny', (t) => {
  const tickets = '{"tickets":[{"id":"T1","rails":["src/types/api.d.ts"]}]}';
  assert.equal(runRails(tickets, 'src/types/api.d.ts', {}, t).verdict, 'deny');
});
...
test("a COMPLETED ticket's rail no longer freezes → allow", (t) => {
  const tickets = '{"tickets":[{"id":"T1","completed":true,"rails":["packages/**"]}]}';
  assert.equal(runRails(tickets, 'packages/fleet/package.json', {}, t).verdict, 'allow');
});

test("an INCOMPLETE ticket's rail still denies (regression guard)", (t) => {
  const tickets = '{"tickets":[{"id":"T1","completed":false,"rails":["packages/**"]}]}';
  assert.equal(runRails(tickets, 'packages/fleet/package.json', {}, t).verdict, 'deny');
});

test('the issue #162 repro: completed T37 freezes packages/**, active T42 rails packages/core/** only', (t) => {
  ...
  assert.equal(runRails(tickets, 'packages/core/lib/x.mjs', {}, t).verdict, 'deny');
});

test('trust root stays frozen even when EVERY declaring ticket is completed', (t) => {
  const tickets = '{"tickets":[{"id":"T1","completed":true,"rails":["packages/**"]}]}';
  assert.equal(runRails(tickets, '.adlc/tickets.json', {}, t).verdict, 'deny');
  assert.equal(runRails(tickets, '.adlc/current-ticket.json', {}, t).verdict, 'deny');
});
...
for (const [name, value] of [
  ...
  test(`completed: ${name} is NOT a completion → rail still denies`, (t) => {
    const tickets = `{"tickets":[{"id":"T1","completed":${value},"rails":["packages/**"]}]}`;
    assert.equal(runRails(tickets, 'packages/fleet/package.json', {}, t).verdict, 'deny');
  });
}
...
test('a COMPLETED ticket with a malformed rails field still fails closed', (t) => {
  const r = runRails('{"tickets":[{"id":"T1","completed":true,"rails":"packages/**"}]}', 'src/app.mjs', {}, t);
  assert.equal(r.verdict, 'deny');
  assert.match(r.out, RAILS_VALIDATION_DENIAL);
});

test('a COMPLETED ticket with a non-string rail entry still fails closed', (t) => {
  const r = runRails('{"tickets":[{"id":"T1","completed":true,"rails":["ok/**",42]}]}', 'src/app.mjs', {}, t);
  assert.equal(r.verdict, 'deny');
  assert.match(r.out, RAILS_VALIDATION_DENIAL);
});

test('a COMPLETED ticket with a non-array rails object still fails closed', (t) => {
  const r = runRails('{"tickets":[{"id":"T1","completed":true,"rails":{"glob":"packages/**"}}]}', 'packages/x.mjs', {}, t);
  assert.equal(r.verdict, 'deny');
  assert.match(r.out, RAILS_VALIDATION_DENIAL);
...
test('#243: a frozen-rail denial names its structured-edit scope and honestly scopes the CI backstop', (t) => {
  const r = runRails('{"tickets":[{"id":"T1","rails":["test/auth/**"]}]}', 'test/auth/login.test.mjs', {}, t);
  assert.equal(r.verdict, 'deny');
...
test('#243: the active-ticket pointer denial does not falsely claim CI catches a shell write to it', (t) => {
  ...
  const r = runRails('{"tickets":[{"id":"T1","rails":["test/auth/**"]}]}', '.adlc/current-ticket.json', {}, t);
  assert.equal(r.verdict, 'deny');
  assert.doesNotMatch(r.out, /does not escape enforcement/i);
...
test('editing .adlc/tickets.json while rails exist → deny (trust root)', (t) => {
  assert.equal(runRails(RAIL_T, '.adlc/tickets.json', {}, t).verdict, 'deny');
});

test('editing .adlc/tickets.json with NO rails declared → allow (authoring the first ticket)', (t) => {
  assert.equal(runRails('{"tickets":[]}', '.adlc/tickets.json', {}, t).verdict, 'allow');
});
...
test('editing .adlc/current-ticket.json while rails exist → deny (trust root)', (t) => {
  assert.equal(runRails(RAIL_T, '.adlc/current-ticket.json', {}, t).verdict, 'deny');
});

test('editing .adlc/current-ticket.json with NO rails declared → allow (build-gate not opted into rails)', (t) => {
  assert.equal(runRails('{"tickets":[]}', '.adlc/current-ticket.json', {}, t).verdict, 'allow');
});

// ---- multi-file / nested-path edit payloads ----

/** Run the hook with an arbitrary tool_input payload; returns 'deny'|'allow'. */
function runPayload(ticketsJson, toolInput, options = {}, t = null) {
  let opts = options;
  let ctx = t;
  if (options && typeof options?.after === 'function' && t === null) {
    ctx = options;
  ...
  const { env = {}, cwdOverride = null } = opts;
...
test('edits[] array carrying a rail file_path → deny (MultiEdit-style)', (t) => {
  const v = runPayload(RAIL_MF, { edits: [{ file_path: 'src/types/api.d.ts', old_string: 'a', new_string: 'b' }] }, {}, t);
  assert.equal(v, 'deny');
});

test('files[] string array containing a rail → deny', (t) => {
  const v = runPayload(RAIL_MF, { files: ['src/app.mjs', 'test/auth/x.test.mjs'] }, {}, t);
  assert.equal(v, 'deny');
});

test('multi-file edit with NO rail among the paths → allow', (t) => {
  const v = runPayload(RAIL_MF, { files: ['src/app.mjs', 'docs/readme.md'] }, {}, t);
  assert.equal(v, 'allow');
});

test('chdir failure (unreachable project dir) in rails mode → fail closed', (t) => {
  const v = runPayload(RAIL_MF, { file_path: 'src/app.mjs' }, { cwdOverride: '/nonexistent-adlc-xyz-12345' }, t);
  assert.equal(v, 'deny');
});

// ---- robustness: subdir invocation + symlinked rail definitions ----

test('a RELATIVE file_path from a subdir resolves against the invocation dir → deny', (t) => {
  const dir = tmp(t, 'adlc-rails-');
...
test('RELATIVE file_path with CLAUDE_PROJECT_DIR set to the root resolves against input.cwd → deny', (t) => {
  const dir = tmp(t, 'adlc-rails-');
  mkdirSync(join(dir, '.adlc'));
  writeFileSync(join(dir, '.adlc', 'tickets.json'), '{"tickets":[{"id":"T1","rails":["src/**"]}]}');
  mkdirSync(join(dir, 'src'));
...
test('a nested subdir .adlc does NOT shadow the git-root rail config → deny', (t) => {
  const dir = tmp(t, 'adlc-rails-');
...
test('hook invoked from a SUBDIR (no CLAUDE_PROJECT_DIR) still gates a rail → deny', (t) => {
  const dir = tmp(t, 'adlc-rails-');
  mkdirSync(join(dir, '.adlc'));
  mkdirSync(join(dir, 'src'));
  writeFileSync(join(dir, '.adlc', 'tickets.json'), '{"tickets":[{"id":"T1","rails":["src/**"]}]}');
...
test('rail on a symlinked dir, editing the REAL path directly → deny (resolved rail form)', (t) => {
  const dir = tmp(t, 'adlc-rails-');
  mkdirSync(join(dir, '.adlc'));
...
test('a backslash-spelled rail glob is normalized → deny', (t) => {
  // rail "src\\api.d.ts" (JSON-escaped) → normalized to src/api.d.ts
  const v = runRails('{"tickets":[{"id":"T1","rails":["src\\\\api.d.ts"]}]}', 'src/api.d.ts', {}, t);
  assert.equal(v.verdict, 'deny');
});

test('a rail defined on a symlinked FILE → editing the symlink path → deny (lexical form)', (t) => {
  const dir = tmp(t, 'adlc-rails-');
  mkdirSync(join(dir, '.adlc'));
  mkdirSync(join(dir, 'src'));
...
test('a rail defined on a SYMLINKED dir matches the resolved target → deny', (t) => {
  const dir = tmp(t, 'adlc-rails-');
  mkdirSync(join(dir, '.adlc'));
  mkdirSync(join(dir, 'real_dir'));
  symlinkSync(join(dir, 'real_dir'), join(dir, 'symdir')); // symdir → real_dir
...
test('editing a symlink that resolves to a rail file → deny', (t) => {
  const dir = tmp(t, 'adlc-rails-');
  mkdirSync(join(dir, '.adlc'));
  writeFileSync(join(dir, '.adlc', 'tickets.json'), '{"tickets":[{"id":"T1","rails":["src/api.d.ts"]}]}');
  mkdirSync(join(dir, 'src'));
...
test('writing a NEW nested file under a symlinked rail dir → deny', (t) => {
  const dir = tmp(t, 'adlc-rails-');
  mkdirSync(join(dir, '.adlc'));
...
test('structured edit (Edit) with no extractable path while rails exist → fail closed', (t) => {
  const dir = tmp(t, 'adlc-rails-');
  mkdirSync(join(dir, '.adlc'));
...
test('writing under a BROKEN symlink that points at a (not-yet-existing) rail dir → deny', (t) => {
  const dir = tmp(t, 'adlc-rails-');
  mkdirSync(join(dir, '.adlc'));
...
test('an UNRECOGNIZED matched tool targeting a rail → deny (fail closed by default)', (t) => {
...
test('editing the real target of a symlinked .adlc/tickets.json → deny (trust root via resolved path)', (t) => {
  const dir = tmp(t, 'adlc-rails-');
  mkdirSync(join(dir, '.adlc'));
...
test('a symlink loop in the target path → fail closed (no infinite resolution)', (t) => {
  const dir = tmp(t, 'adlc-rails-');
  mkdirSync(join(dir, '.adlc'));
  writeFileSync(join(dir, '.adlc', 'tickets.json'), '{"tickets":[{"id":"T1","rails":["test/**"]}]}');
...
for (const [name, raw] of [
  ...
  test(`canonicalize: ${name} still denies the rail`, (t) => {
    const tickets = '{"tickets":[{"id":"T1","rails":["src/types/api.d.ts"]}]}';
    assert.equal(runRails(tickets, '', { rawFilePath: raw }, t).verdict, 'deny');
  });
}

test('canonicalize: relative ./ input resolves against the repo and denies', (t) => {
  const tickets = '{"tickets":[{"id":"T1","rails":["src/types/api.d.ts"]}]}';
  assert.equal(runRails(tickets, '', { rawFilePath: './src/types/api.d.ts' }, t).verdict, 'deny');
});
...
for (const [name, json] of [
  ...
  test(`fail closed: ${name} → deny`, (t) => {
    assert.equal(runRails(json, 'src/app.mjs', {}, t).verdict, 'deny');
  });
}
...
test('bypass on a rail WITH a working recorder → allow + audited entry', (t) => {
  const tickets = '{"tickets":[{"id":"T1","rails":["test/**"]}]}';
  const r = runRails(tickets, 'test/x.mjs', { env: { ADLC_RAILS_BYPASS: '1', PATH: WITH_ADLC } }, t);
  assert.equal(r.verdict, 'allow');
  assert.match(r.manifest, /rails-bypass/);
});

test('bypass on schema-invalid tickets WITH recorder → allow (audited)', (t) => {
  const r = runRails('[]', 'src/app.mjs', { env: { ADLC_RAILS_BYPASS: '1', PATH: WITH_ADLC } }, t);
  assert.equal(r.verdict, 'allow');
});

test('bypass with the recorder UNAVAILABLE → deny (an unaudited override is refused)', (t) => {
  ...
  const noAdlcDir = tmp(t, 'adlc-no-recorder-');
  const tickets = '{"tickets":[{"id":"T1","rails":["test/**"]}]}';
  const r = runRails(tickets, 'test/x.mjs', { env: { ADLC_RAILS_BYPASS: '1', PATH: noAdlcDir } }, t);
  assert.equal(r.verdict, 'deny');
});

test('bypass on a multi-file edit hitting two rails → allow + BOTH audited', (t) => {
  const dir = tmp(t, 'adlc-rails-');
  mkdirSync(join(dir, '.adlc'));
...
test('#204 (AC1): an ambient ADLC_RAILS_BYPASS in the parent env does not disarm a deny-test', (t) => {
...
test('#204 (AC2): a scoped ADLC_RAILS_BYPASS=<glob> authorizes only matching rails', (t) => {
  const tickets = '{"tickets":[{"id":"T1","rails":["test/auth/**","src/types/api.d.ts"]}]}';
  // In scope → allow (and audited).
  const rIn = runRails(tickets, 'test/auth/login.test.mjs', { env: { ADLC_RAILS_BYPASS: 'test/auth/**', PATH: WITH_ADLC } }, t);
  ...
  const rOut = runRails(tickets, 'src/types/api.d.ts', { env: { ADLC_RAILS_BYPASS: 'test/auth/**', PATH: WITH_ADLC } }, t);
  assert.equal(rOut.verdict, 'deny');
  assert.match(rOut.out, /does not authorize/i);
});

test('#204 (AC3): the unscoped =1 bypass emits a visible notice disclosing its session lifetime', (t) => {
  const tickets = '{"tickets":[{"id":"T1","rails":["test/auth/**"]}]}';
  const r = runRails(tickets, 'test/auth/login.test.mjs', { env: { ADLC_RAILS_BYPASS: '1', PATH: WITH_ADLC } }, t);
  assert.equal(r.verdict, 'allow');
...
test('#204 (AC3): a scoped bypass notice names the scope and omits the session-wide disclosure', (t) => {
  const tickets = '{"tickets":[{"id":"T1","rails":["test/auth/**"]}]}';
  const r = runRails(tickets, 'test/auth/login.test.mjs', { env: { ADLC_RAILS_BYPASS: 'test/auth/**', PATH: WITH_ADLC } }, t);
  assert.equal(r.verdict, 'allow');
...
test('#204: an inert sentinel (=0 / =false / =off) is NOT a bypass — a rail edit still denies', (t) => {
  const tickets = '{"tickets":[{"id":"T1","rails":["test/auth/**"]}]}';
  for (const v of ['0', 'false', 'off']) {
    const r = runRails(tickets, 'test/auth/login.test.mjs', { env: { ADLC_RAILS_BYPASS: v, PATH: WITH_ADLC } }, t);
...
test('#204: a wildcard-only scope (=** / =* / =**/*) is refused — session-wide requires =1', (t) => {
  const tickets = '{"tickets":[{"id":"T1","rails":["test/auth/**"]}]}';
  for (const v of ['**', '*', '**/*']) {
    const r = runRails(tickets, 'test/auth/login.test.mjs', { env: { ADLC_RAILS_BYPASS: v, PATH: WITH_ADLC } }, t);
...
test('a bare .adlc directory above a repo is not adopted as the ADLC root', (t) => {
  const home = tmp(t, 'adlc-home-bare-');
...
test('a ticket store that LANDS at ~/.adlc cannot capture a repo below it', (t) => {
  const home = tmp(t, 'adlc-home-store-');
...
test('a repo rooted AT $HOME still enforces its own rails', (t) => {
  const home = tmp(t, 'adlc-home-dotfiles-');
  mkdirSync(join(home, '.adlc'));
  writeFileSync(
    join(home, '.adlc', 'tickets.json'),
...
test('a dotfiles repo rooted at $HOME enforces rails from a SUBDIRECTORY', (t) => {
  const home = tmp(t, 'adlc-home-dotfiles-sub-');
...
test('a stray store at $HOME is still ignored even when $HOME is a git repo below it', (t) => {
  // $HOME is NOT a git repo here — only the nested project is. The store at
  // $HOME must not reach down into it.
  const home = tmp(t, 'adlc-home-stray-sub-');
  mkdirSync(join(home, '.adlc'), { recursive: true });
  writeFileSync(
    join(home, '.adlc', 'tickets.json'),
    JSON.stringify({ tickets: [{ id: 'T1', title: 'stray', rails: ['**'] }] }),
...
test('the nearest-ancestor store wins over a higher one when there is no git root', (t) => {
  const box = tmp(t, 'adlc-nested-store-');
...
test('a store in an ancestor ABOVE a non-ADLC git repo does not capture it', (t) => {
  const home = tmp(t, 'adlc-ancestor-store-');
...
test('a package-level store BELOW the git root still enforces its rails', (t) => {
  const home = tmp(t, 'adlc-monorepo-store-');
