...
// deadline check in boundedTailRead (adlc-handoff-gate.mjs) only ran BETWEEN
...
// by boundedTailRead via `spawnSync(..., { timeout })`. The OS can forcibly
// kill this process if it does not finish in time — real preemption, which
// nothing in-process can offer against a blocking syscall. Keeping the read
