...
You have no edit/execute tools by design: this lens only reads the diff and test
files (via `read` and `search`) and reasons about it — it does not run the test
suite itself (that is `adlc hollow-test`'s job). It never changes anything and
never shells out.
