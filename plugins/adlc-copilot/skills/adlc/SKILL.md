...
---
<!-- ADLC_COPILOT_SENTINEL_PHASE_ROUTER_V1 -->
...
**Copilot is the model.** Every LLM-backed gate supports `--prompt-only`: it
prints the exact prompt and exits without calling any provider. Inside GitHub
Copilot CLI you do not need API keys — run the tool with `--prompt-only`, answer
the printed prompt yourself, and apply the judgment. Prefer this over wiring keys.
...
- `adlc coldstart <ticket-id> --prompt-only` (or `--all`) — gate ticket
  executability. LLM-backed: in Copilot, use `--prompt-only` and answer the
