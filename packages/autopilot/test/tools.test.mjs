...
function fixture(t) {
  ...
  for (const tool of REQUIRED_TOOLS) writeFileSync(join(sys, tool), '#!/bin/sh\n'), chmodSync(join(sys, tool), 0o755);
...
export function ac68_repoBinIsSkippedAndSystemPinned(t) {
  const { root, repoRoot, sys } = fixture(t);
  ...
  for (const tool of REQUIRED_TOOLS) assert.equal(pinned[tool], join(sys, tool), `${tool} pinned absolute`);
...
export function ac68_untrustedTool(t) {
  const { root, repoRoot, sys } = fixture(t);
...
export function ac68_trustedBinDirsNarrows(t) {
  const { root, repoRoot, sys } = fixture(t);
  const stat = () => ({ uid, mode: 0o755 });
  const alt = join(root, 'alt'); mkdirSync(alt); for (const tool of REQUIRED_TOOLS) writeFileSync(join(alt, tool), '');
