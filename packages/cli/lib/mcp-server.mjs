...
const CLI_BIN = resolve(CLI_ROOT, 'bin/adlc.mjs');
const VERSION = JSON.parse(readFileSync(resolve(CLI_ROOT, 'package.json'), 'utf8')).version;
...
function command() {
  const bin = process.env.ADLC_CLI_BIN ? resolve(process.env.ADLC_CLI_BIN) : CLI_BIN;
  return { file: process.execPath, prefix: [bin] };
}
...
export function handleRequest(request) {
  ...
  if (request.method === 'initialize') {
    return {
      ...
      capabilities: { tools: {} },
      serverInfo: { name: 'adlc-codex', version: VERSION },
    };
  }
