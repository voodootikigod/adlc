...
| `--record` | off | On a clean verdict, append a `flail-check` entry to `.adlc/manifest.jsonl` (ADLC P4 evidence — see `@adlc/runner`). Never writes on a `flail` verdict, and never on a `could-not-analyze` outcome (issue #622). |
