...
function repo(t, { gitignore = null, init = true } = {}) {
  let root;
  let g;
  if (init) {
    const r = gitRepo(t, { prefix: 'gate-manifest-enable-', branch: 'feat/enable', userEmail: 't@t.co', userName: 'tester' });
    root = r.dir;
    g = r.g;
  } else {
    root = tmp(t, 'gate-manifest-enable-');
  }
...
describe('planEnable decision order (spec Storage modes; ticket work item 1h)', () => {
  it('refuses when no .adlc workspace exists — never creates one as a side effect', (t) => {
    const { root } = repo(t);
  ...
  it('reports already-enabled for a marker-carrying repo', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('AC4: reports already-enabled for a cutover-tailed root with a MISSING marker — never tells a cut-over repo to re-run a ceremony it already ran', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('AC4: refuses a manifest.d/ that has content but no valid marker (broken state to surface, not silently repair)', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    mkdirSync(join(dir, 'manifest.d'), { recursive: true });
  ...
  it('refuses a SYMLINKED root manifest without reading through it — same no-follow policy as every other path component', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    const external = tmp(t, 'enable-root-symlink-');
  ...
  it('AC2: refuses a live (non-empty, non-cutover) root, naming the cutover ceremony ticket', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('AC10: refuses when the marker path is gitignored — an ignored marker silently reverts every other checkout to single-file mode', (t) => {
    const { root, dir } = repo(t, { gitignore: IGNORED });
    const plan = planEnable(dir, { cwd: root });
  ...
  it('proceeds greenfield with an ABSENT root and with an EMPTY root when the full gitignore contract holds', (t) => {
  ...
  it('refuses a repo with NO .gitignore — the checkout-local token would be trackable, and a committed token poisons every clone', (t) => {
    const { root, dir } = repo(t, { gitignore: null });
    const plan = planEnable(dir, { cwd: root });
    assert.equal(plan.decision, 'refuse-ignored');
  ...
  it('the .lineage token stays untracked end-to-end: enable, first write, then git add -A stages segments but never the token', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('proceeds outside a git repository — evidence without git is supported, so the probe soft-passes', (t) => {
    const { root, dir } = repo(t, { init: false });
    const plan = planEnable(dir, { cwd: root });
...
describe('enable() write path (AC1, AC3, AC4)', () => {
  it('AC3: dry-run (write: false) changes nothing on disk', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('AC1: write flips the repo to forest mode, recognized by BOTH packages, and the next append writes a segment, not root', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('AC4: idempotent — a second enable exits already-enabled and leaves the marker bytes untouched', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('AC10: the remediation advice WORKS for BOTH ignore styles — appending exactly the advertised negation lines converts the refusal into a committable enable', (t) => {
    ...
    for (const style of ['.adlc/*\n', '.adlc/**\n', '*.jsonl\n']) {
      const { root, dir } = repo(t, { gitignore: style });
  ...
  it('AC10: a segment-targeting ignore rule refuses even though the dir and marker probes alone would pass', (t) => {
  ...
  it('refuses a symlinked WORKSPACE (.adlc itself) — same no-follow policy as the inner directory', (t) => {
    const { root } = repo(t, { gitignore: NEGATED });
    const external = tmp(t, 'enable-workspace-target-');
    rmSync(join(root, '.adlc'), { recursive: true });
  ...
  it('a failed marker write strands NO temp file and leaves the repo still cleanly greenfield', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('refuses a symlinked manifest.d even when its TARGET contains a valid marker — never reports already-enabled through a link', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    const external = tmp(t, 'enable-symlink-marker-');
  ...
  it('refuses a symlinked manifest.d and leaves the external target untouched — never writes through the link', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    const external = tmp(t, 'enable-symlink-target-');
    symlinkSync(external, join(dir, 'manifest.d'));
    const out = enable(dir, { write: true });
    assert.equal(out.decision, 'refuse-broken-manifest-dir');
    assert.match(out.reason, /symlink|not a real directory/);
  ...
  it('AC10: a marker-SPECIFIC ignore rule refuses even though the directory probe alone would pass', (t) => {
    const { root, dir } = repo(t, { gitignore: '.adlc/manifest.d/.store.json\n' });
    const out = enable(dir, { cwd: root, write: true });
  ...
  it('the gitignore probe never leaks the manifest key or GIT_* repo selectors to the git child', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    const shimDir = tmp(t, 'git-shim-');
  ...
  it('the marker is published under the root-ledger lock — a held lock blocks enable instead of racing a concurrent root append', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('refusals write nothing even with write: true', (t) => {
    const { root, dir } = repo(t, { gitignore: IGNORED });
    const before = snapshot(dir);
    const out = enable(dir, { cwd: root, write: true });
...
describe('gate-manifest enable CLI (AC2, AC3, AC10, AC11)', () => {
  it('dry-run greenfield: exit 0, plan on stdout, nothing created', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    const r = runBin(root);
  ...
  it('--write greenfield: exit 0, marker exists afterwards', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('AC2: live root refuses with exit 2, stderr names the cutover ceremony, and creates nothing', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    writeFileSync(join(dir, 'manifest.jsonl'), `${JSON.stringify({ seq: 1, gate: 'evidence', ts: '2026-01-01T00:00:00.000Z', files: {}, prev: null })}\n`);
  ...
  it('AC10: gitignored marker refuses with exit 2 in BOTH modes and creates nothing', (t) => {
    const { root, dir } = repo(t, { gitignore: IGNORED });
    for (const args of [[], ['--write']]) {
      const r = runBin(root, ...args);
      assert.equal(r.status, 2, `args=${args.join(' ')}`);
  ...
  it('no workspace: exit 2 pointing at init', (t) => {
    const { root } = repo(t);
    rmSync(join(root, '.adlc'), { recursive: true });
    const r = runBin(root);
  ...
  it('AC15: KEYLESS activation refuses with exit 2 in both modes — the trap is one-way, so it must be opted into, never stumbled into', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    for (const args of [[], ['--write']]) {
  ...
  it('AC15: the marker PERSISTS keyed mode, and a later keyless write is refused before touching anything — the invariant lives in the repository, not the shell', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    execFileSync('git', ['add', '.gitignore'], { cwd: root, stdio: ['ignore', 'pipe', 'ignore'] });
    execFileSync('git', ['commit', '-q', '-m', 'init'], { cwd: root, stdio: ['ignore', 'pipe', 'ignore'] });
  ...
  it('AC15: an --allow-keyless activation persists keyless mode, and keyless writes keep working', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    execFileSync('git', ['add', '.gitignore'], { cwd: root, stdio: ['ignore', 'pipe', 'ignore'] });
    execFileSync('git', ['commit', '-q', '-m', 'init'], { cwd: root, stdio: ['ignore', 'pipe', 'ignore'] });
  ...
  it('AC15: keyless --json refusal is a single parseable document with decision refuse-keyless', (t) => {
    const { root } = repo(t, { gitignore: NEGATED });
  ...
  it('AC10: an ALREADY-ENABLED repo whose ignore rules drifted refuses as a health check — never reports success while the token is trackable', (t) => {
    for (const [gitignore, hazard] of [[IGNORED, /marker|segments/], [null, /TRACK|lineage token/]]) {
      const { root, dir } = repo(t, { gitignore });
      mkdirSync(join(dir, 'manifest.d'), { recursive: true });
      writeFileSync(markerPath(dir), JSON.stringify({ format: 'adlc-manifest-segments', version: 1 }));
      const plan = planEnable(dir, { cwd: root });
  ...
  it('AC11: --json emits exactly one parseable document — dry-run, write, and refusal alike', (t) => {
    const cases = [
    ...
    for (const c of cases) {
      const { root } = repo(t, { gitignore: c.gitignore });
...
describe('enable discloses that CI cannot yet guard segments', () => {
  it('AC1: a greenfield activation carries the warning', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    const plan = planEnable(dir, { cwd: root });
    assert.equal(plan.decision, 'greenfield');
  ...
  it('AC2: a repo already in forest mode carries the warning too — it is writing segments right now', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    enable(dir, { write: true });
    const plan = planEnable(dir, { cwd: root });
    assert.equal(plan.decision, 'already-enabled');
  ...
  it('AC3: refusals do NOT carry it — a refused repo is not in forest mode, and noise trains operators to ignore warnings', (t) => {
    const live = repo(t, { gitignore: NEGATED });
    ...
    const planLive = planEnable(live.dir, { cwd: live.root });
    assert.equal(planLive.decision, 'refuse-live-root');
    assert.ok(!hasCiWarning(planLive), 'refuse-live-root must not warn');

    const ignored = repo(t, { gitignore: IGNORED });
    const planIgnored = planEnable(ignored.dir, { cwd: ignored.root });
    assert.equal(planIgnored.decision, 'refuse-ignored');
    assert.ok(!hasCiWarning(planIgnored), 'refuse-ignored must not warn');

    const none = repo(t, { gitignore: NEGATED });
    rmSync(join(none.root, '.adlc'), { recursive: true });
    const planNone = planEnable(join(none.root, '.adlc'), { cwd: none.root });
    assert.equal(planNone.decision, 'refuse-no-workspace');
    assert.ok(!hasCiWarning(planNone), 'refuse-no-workspace must not warn');
  });

  it('AC4: disclosure changes no decision and no write behavior', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('AC5: --json still emits exactly ONE document, and it carries the warning', (t) => {
    const { root } = repo(t, { gitignore: NEGATED });
  ...
  it('AC6: human mode surfaces the warning code visibly', (t) => {
    const { root } = repo(t, { gitignore: NEGATED });
    const r = runBin(root);
    assert.equal(r.status, 0, r.stderr);
...
describe('disclosure follows segmentation, not the decision', () => {
  it('AC7: already-enabled WITH gitignore drift still discloses — it is segmented and exposed', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    enable(dir, { write: true });
  ...
  it('AC8: human mode prints the warning even when the run refuses', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    enable(dir, { write: true });
    writeFileSync(join(root, '.gitignore'), IGNORED);
    const r = runBin(root);
  ...
  it('AC9: a keyless refusal on an ALREADY-segmented repo discloses, in both modes', (t) => {
  ...
  it('AC9b: the keyless refusal still outranks every plan refusal, at exit 2', (t) => {
  ...
  it('AC9c: the probe is no-follow and bounded — it never reads a hostile marker path', (t) => {
    const cases = [
  ...
  it('AC9d: the probe recognizes a genuine marker and never throws on a missing one', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('AC10: a keyless refusal on a NON-segmented repo still does not disclose', (t) => {
    const { root } = repo(t, { gitignore: NEGATED });
    const r = runBinKeyless(root, '--json');
    assert.equal(r.status, 2);
    const parsed = JSON.parse(r.stdout);
...
describe('segmentation is never inferred through a symlink', () => {
  it('AC12: planEnable itself refuses a symlinked root before any content read', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
...
describe('the bounded probe covers the whole segmented-mode invariant', () => {
  ...
  it('AC9e: a cutover-tailed root with NO marker is segmented, and a keyless run discloses', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('AC9f: a NON-cutover tail is not segmented — the probe reads the last entry, not any entry', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('AC9g: bounded and no-follow, and an UNDECIDABLE root is reported as such', (t) => {
  ...
  it('AC9h: the bounded probe NEVER under-reports against the production predicate', (t) => {
...
describe('the tail window is sized so ordinary ledgers stay decidable', () => {
  it('AC9i: a large single-file ledger with realistic entry sizes does not warn', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
  ...
  it('AC9j: a cutover tail is still found when the preceding entries are large', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
...
describe('the emitted code matches what the bounded read proved', () => {
  ...
  it('AC9k: an oversized NON-cutover final entry is undetermined, never claimed as segmented', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    writeFileSync(join(dir, 'manifest.jsonl'),
  ...
  it('AC9l: a definite cutover tail emits the segmented code, not the uncertainty one', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    writeFileSync(join(dir, 'manifest.jsonl'),
  ...
  it('AC9m: an ordinary single-file repo emits neither code', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
    writeFileSync(join(dir, 'manifest.jsonl'),
      `${JSON.stringify({ seq: 1, gate: 'evidence', ts: '2026-01-01T00:00:00.000Z', files: {}, prev: null })}\n`);
...
describe('the probe’s decision boundaries are pinned', () => {
  it('AC9n: a marker must match BOTH format and version, not either', (t) => {
    const cases = [
  ...
  it('AC9o: the tail window boundary is exact — a final line of exactly the window size is undecidable', (t) => {
  ...
  it('AC9p: an absent root is single-file, and only a recognized result may say so', (t) => {
    const { root, dir } = repo(t, { gitignore: NEGATED });
