...
describe('unknown is not zero (T152 no-fabrication rule)', () => {
  it('an unreported call and a genuine all-zero reported call are distinguishable, and only one is counted', (t) => {
    const dir = tmp(t, 'gate-manifest-usage-roundtrip-');
  ...
  it('an unreported entry contributes a bucket that is EXPLICITLY unmeasured, never a zeroed one', (t) => {
...
describe('sibling fields (channel, transport, registryDigest)', () => {
  ...
  it('are preserved verbatim through the real ledger roundtrip', (t) => {
    const dir = tmp(t, 'gate-manifest-usage-roundtrip-');
  ...
  it('omit cleanly — an entry recorded without them carries no empty placeholders', (t) => {
    const dir = tmp(t, 'gate-manifest-usage-roundtrip-');
  ...
  it('leave aggregateSpend byte-identical — siblings are recorded, never counted', (t) => {
    ...
    const withDir = tmp(t, 'gate-manifest-usage-roundtrip-');
    const withoutDir = tmp(t, 'gate-manifest-usage-roundtrip-');
