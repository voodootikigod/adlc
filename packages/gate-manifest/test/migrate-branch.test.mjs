 ...
 */
function scenario(t, { branchEntries = [evidence(10), approveEntry()], onBranch = 'feat-x' } = {}) {
  const root = tmp(t, 'gm-salvage-');
...
describe('refusals', () => {
  it('keyless refuses — salvage exists to preserve AUTHENTICATED evidence', (t) => {
    const { root, dir, sourceSha } = scenario(t);
    const plan = planMigrateBranch(dir, { key: null, sourceRef: sourceSha, cwd: root });
    assert.equal(plan.decision, 'refuse-keyless');
  });

  it('a non-segmented repository refuses — salvage is a post-cutover tool', (t) => {
    const root = tmp(t, 'gm-salvage-flat-');
  ...
  it('an unresolvable source ref refuses', (t) => {
    const { root, dir } = scenario(t);
    const plan = planMigrateBranch(dir, { key: KEY, sourceRef: 'no-such-ref-anywhere', cwd: root });
    assert.equal(plan.decision, 'refuse-source');
  });

  it('a working root still carrying the branch tail refuses with the take-main recipe', (t) => {
    const { root, dir, sourceSha, g } = scenario(t);
  ...
  it('a TAMPERED source entry refuses, and --attest-unsigned does NOT admit it — only genuinely unsigned entries', (t) => {
    const tampered = { gate: 'evidence', data: { note: 'tampered-later' }, files: {} };
    const { root, dir, sourceSha, g } = scenario(t, { branchEntries: [tampered] });
  ...
  it('a branch that already owns a segment refuses — salvage runs once, before new writes', (t) => {
    const { root, dir, sourceSha } = scenario(t);
  ...
  it('a source suffix that does not chain from the shared prefix refuses — corruption surfaced, not truncated', (t) => {
    const { root, dir, g } = scenario(t);
...
describe('the salvage', () => {
  it('dry-run plans (count, branch, source) and writes nothing', (t) => {
    const { root, dir, sourceSha } = scenario(t);
  ...
  it('end-to-end: content preserved, root untouched, forest verifies, reader finds the approve, the CI gate accepts', (t) => {
    const { root, dir, sourceSha } = scenario(t);
  ...
  it('genuinely UNSIGNED source entries are admitted only with --attest-unsigned, and disclosed', (t) => {
    const { root, dir, g } = scenario(t);
...
describe('signature-coverage, race detection, and resumability', () => {
  it('a v1-signed entry carrying a field its signature never covered refuses — no laundering into v2', async (t) => {
    const { root, dir, sourceSha, g } = scenario(t);
  ...
  it('an interrupted salvage RESUMES: already-appended prefix is skipped, the record still lands', (t) => {
    const { root, dir, sourceSha } = scenario(t);
  ...
  it('a branch segment holding entries UNRELATED to the salvage still refuses', (t) => {
    const { root, dir, sourceSha } = scenario(t);
...
describe('round-2 hardening', () => {
  it('a flag-shaped source ref refuses before reaching git', (t) => {
    const { root, dir } = scenario(t);
  ...
  it('a completed IDENTICAL salvage refuses as done; a MISMATCHED record refuses as inspect-first', (t) => {
    const { root, dir, sourceSha } = scenario(t);
...
describe('mutation kill coverage', () => {
  ...
  it('the migrate-branch verb: refusal exits 2 (json), human dry-run exits 0 with correct pluralization', (t) => {
    const { root, sourceSha } = scenario(t);
  ...
  it('a clean v1-signed source entry salvages — its own sig field is a covered field, not contraband', async (t) => {
    const { root, dir, g } = scenario(t);
    const { signEntry } = await import('../lib/sign.mjs');
    const working = readFileSync(join(dir, 'manifest.jsonl'), 'utf8');
    const sharedLines = working.trim().split('\n').slice(0, 2);
