...
function liveRepo(t, { entries = [], gitignore = true } = {}) {
  const root = tmp(t, 'gm-migrate-');
...
describe('refusals (spec §8 steps 1–4)', () => {
  it('refuses without a key — the ceremony signs and verifies, no keyless form exists', (t) => {
    const { root, dir } = liveRepo(t, { entries: [evidence()] });
  ...
  it('refuses an invalid chain, naming repair-chain', (t) => {
    const { root, dir } = liveRepo(t, { entries: [evidence(), evidence()] });
  ...
  it('refuses unsigned entries without --attest-unsigned, and discloses them with it', (t) => {
    const { root, dir } = liveRepo(t);
  ...
  it('refuses when already segmented — by marker AND by cutover tail alone', (t) => {
    // marker-only
    const a = liveRepo(t);
    ...
    const b = liveRepo(t, { entries: [evidence()] });
  ...
  it('refuses a reason shorter than 8 characters', (t) => {
    const { root, dir } = liveRepo(t, { entries: [evidence()] });
    const plan = planMigrate(dir, { key: KEY, reason: 'short' });
    assert.equal(plan.decision, 'refuse-reason');
  });

  it('refuses when the gitignore contract would strand the marker uncommittable', (t) => {
    const { root, dir } = liveRepo(t, { entries: [evidence()], gitignore: false });
...
describe('the plan (step 5) and the write (step 6)', () => {
  it('dry-run lists sealed tuples, cutover fields, backup and marker paths — and writes NOTHING', (t) => {
    const { root, dir } = liveRepo(t, { entries: [evidence(), approve({ ticket: 'T-X' })] });
  ...
  it('the write: backup byte-identical, root appended-only, marker keyed, verify() green', (t) => {
    const { root, dir } = liveRepo(t, { entries: [evidence(), approve()] });
  ...
  it('THE decisive test: the ceremony output passes rails-guard\'s own merged validators', (t) => {
    ...
    const { root, dir } = liveRepo(t, { entries });
  ...
  it('a mid-ceremony crash (seals appended, no cutover) recovers on re-run without duplicate seals', (t) => {
    const { root, dir } = liveRepo(t, { entries: [evidence(), approve()] });
  ...
  it('the crash window between cutover and marker: re-run refuses, enable reports already-enabled', (t) => {
    const { root, dir } = liveRepo(t, { entries: [evidence()] });
...
describe('the reader agrees, and bytes stay bytes', () => {
  it('after the ceremony every previously-standing approve is revoked for the forest reader', async (t) => {
    const { root, dir } = liveRepo(t, { entries: [evidence(), approve({ ticket: 'T-R' }), approve({ data: { revision: 'git-change:base:d2' } })] });
    migrate(dir, { key: KEY, reason: REASON, write: true });
  ...
  it('a legacy region with invalid utf8 still produces a byte-exact rootSha256 the gate accepts', (t) => {
    const { root, dir } = liveRepo(t);
...
describe('write-path hardening', () => {
  it('a symlinked root manifest refuses — the ceremony must not write through repository-controlled links', (t) => {
    const { root, dir } = liveRepo(t);
    const external = tmp(t, 'gm-migrate-ext-');
  ...
  it('a second migration racing the first refuses INSIDE the lock — no duplicate cutover', (t) => {
    const { root, dir } = liveRepo(t, { entries: [evidence(), approve()] });
    migrate(dir, { key: KEY, reason: REASON, write: true });
  ...
  it('a root WITHOUT a trailing newline gains one before the appendix — no line concatenation corruption', (t) => {
    const { root, dir } = liveRepo(t, { entries: [evidence()] });
...
describe('backup-path and durability hardening', () => {
  it('a pre-planted symlink at the predictable backup path refuses — nothing is written through it', (t) => {
    const { root, dir } = liveRepo(t, { entries: [evidence()] });
    const external = tmp(t, 'gm-migrate-bk-');
...
describe('CLI surface and normalization discrimination', () => {
  ...
  it('the migrate verb: refusal exits 2 with one JSON document; dry-run exits 0', (t) => {
    const { root } = liveRepo(t, { entries: [evidence()] });
  ...
  it('distinct providers at one revision stay DISTINCT tuples; whitespace/case variants of one provider MERGE', (t) => {
