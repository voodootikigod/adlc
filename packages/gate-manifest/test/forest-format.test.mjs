...
import { writeFileSync, mkdirSync, symlinkSync, readdirSync } from 'node:fs';
...
describe('forest verify() — segment-free fast path (AC2)', () => {
  it('empty manifest, no manifest.d/, verifies exactly as the pre-forest single chain', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
  ...
  it('a root-only manifest with entries verifies with segments:0', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
  ...
  it('an empty manifest.d/ directory (no segments yet) still takes the fast path', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
  ...
  it('a broken root chain on the fast path still labels break.segment as root (contract consistency)', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
...
describe('forest verify() — a valid forest', () => {
  it('root + one segment anchored to root verifies, segments:1', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('root-less: two independent anchor:null segments both verify (§4.4/§9.3 concurrent-branch case)', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('signed:true only when every chain in the forest verifies signed', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('a legacy-unsigned root prefix is tolerated per-chain, independent of a fully-signed segment', (t) => {
  ...
  it('an unsigned entry AFTER the root has gone signed-era still breaks the chain (no regression via appended plain entries)', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
    const rootLines = buildChainLines(
  ...
  it('root-less repo with all-signed segments reports signed:true (empty root must not drag it down)', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
...
describe('forest verify() — AC1 adversarial rejections', () => {
  it('a manifest.d/ that exists as a regular file (not a directory) fails closed, does not crash', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects malformed JSON in a segment', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects a segment whose first entry has seq !== 1 (AC4)', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects a segment with non-contiguous sequence numbers (AC4)', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects bad filename grammar', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects a lowercase ULID (grammar requires uppercase, no folding)', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects a symlinked segment file', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects a symlinked manifest.d/ directory itself', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects a DANGLING symlinked manifest.d/ (target does not exist) — existsSync would miss this', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
    mkdirSync(adlc, { recursive: true });
  ...
  it('rejects a nested directory under manifest.d/', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('a name that is a pure-case variant of a valid segment is rejected (as bad grammar, not silently accepted)', (t) => {
  ...
  it('rejects a dangling anchor (points at a segment that does not exist)', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects an anchor whose seq does not exist in the target chain', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects a lineHash mismatch (anchor to a real seq, wrong hash — repointed history)', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
    const rootLines = buildChainLines([{ gate: 'x' }]);
    writeLines(join(adlc, 'manifest.jsonl'), rootLines);
  ...
  it('rejects an anchor cycle (two segments anchored to each other)', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects anchor:null once a root exists', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects a segment first entry missing the required anchor field', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects a genuinely empty segment file (no first entry, so no anchor at all)', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects an anchor field on a non-first segment entry', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('rejects an anchor field on a root entry', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
...
describe('readManifestForest — ordering', () => {
  it('a segment anchored to another segment sorts AFTER its parent, even when its name string-sorts first', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('two root-less segments sharing a ULID (different slugs) still sort deterministically', (t) => {
  ...
  it('a valid-JSON non-object line (null, a number, a bare array) is skipped, not fabricated into a phantom entry', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
...
describe('discoverSegments — grammar edge cases', () => {
  it('accepts a slug containing every digit 0-9, not just 0-1 (full character-class range)', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
  ...
  it('.lineage is a structural file, not a segment — skipped, never reported as invalid', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
    mkdirSync(join(adlc, 'manifest.d'), { recursive: true });
  ...
  it('a live *.lock file with a genuine owner-record body is a structural artifact, not a segment — skipped', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
    mkdirSync(join(adlc, 'manifest.d'), { recursive: true });
    writeLines(join(adlc, 'manifest.d', 'feat-01ARZ3NDEKTSV4RRFFQ69G5FAV.jsonl'), buildChainLines([{ gate: 'x', anchor: null }]));
  ...
  // in-flight write.
  it('a *.lock file that is transiently EMPTY (mid-creation) is treated as a genuine lock, not invalid', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
    mkdirSync(join(adlc, 'manifest.d'), { recursive: true });
    writeLines(join(adlc, 'manifest.d', 'feat-01ARZ3NDEKTSV4RRFFQ69G5FAV.jsonl'), buildChainLines([{ gate: 'x', anchor: null }]));
  ...
  it('a real segment renamed to end in .lock is NOT silently hidden — its content does not match a genuine lock', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
    mkdirSync(join(adlc, 'manifest.d'), { recursive: true });
  ...
  it('an oversized *.lock file is refused, not treated as a genuine lock', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
    mkdirSync(join(adlc, 'manifest.d'), { recursive: true });
  ...
  it('the lock size-cap boundary is exact: 512 bytes is refused, 511 is accepted', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
    mkdirSync(join(adlc, 'manifest.d'), { recursive: true });
  ...
  it('a *.lock file whose content is not even valid JSON is refused, not treated as a genuine lock', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    const adlc = join(dir, '.adlc');
    mkdirSync(join(adlc, 'manifest.d'), { recursive: true });
...
describe('forest primitives — unit coverage', () => {
  it('discoverSegments reports invalid entries without throwing', (t) => {
    const dir = tmp(t, 'gate-manifest-forest-test-');
    mkdirSync(join(dir, '.adlc', 'manifest.d'), { recursive: true });
