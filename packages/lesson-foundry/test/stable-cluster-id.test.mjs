...
// ---------------------------------------------------------------------------
test('spec-gap: a reworded lesson that dropped the legacy slug marker is still banked via the id', (t) => {
  const dir = tmp(t, 'lesson-foundry-stable-id-');
...
test('spec-gap: a defended cluster STAYS banked when the same pattern recurs again', (t) => {
...
test('spec-gap: a merged OLDER occurrence does not un-bank a defended cluster', (t) => {
...
test('spec-gap: a defended cluster fused with an UNDEFENDED one is still reported unbanked', (t) => {
...
test('lint: a slug-NAMED artifact does not bypass the coverage invariant', (t) => {
...
test('spec-gap: a different cluster sharing a slug is NOT credited by the other lesson', (t) => {
...
test('coverage: uncovered occurrences that do not cluster with EACH OTHER keep the cluster banked', (t) => {
...
test('coverage: uncovered occurrences that DO cluster together surface the cluster', (t) => {
...
test('spec-gap: banking survives cluster members being re-ordered between emit and gate', (t) => {
  const dir = tmp(t, 'lesson-foundry-stable-id-');
  const outDir = join(dir, 'lessons');
  mkdirSync(outDir, { recursive: true });
  const a = { ts: 't1', file: 'a.mjs', line: 1, category: 'security', desc: 'unclear data retention policy across services' };
...
test('skill: a lesson whose file was renamed is still banked via the id in its content', (t) => {
  const dir = tmp(t, 'lesson-foundry-stable-id-');
  const outDir = join(dir, 'lessons');
  mkdirSync(outDir, { recursive: true });
  const findings = [
...
test('lint: a descriptor whose file was renamed is still banked via the id in its content', (t) => {
  const dir = tmp(t, 'lesson-foundry-stable-id-');
  const outDir = join(dir, 'lessons');
  mkdirSync(outDir, { recursive: true });
  const findings = [
...
// ---------------------------------------------------------------------------
test('spec-gap: a genuinely undistilled cluster is still reported unbanked (no id false-credit)', (t) => {
  const dir = tmp(t, 'lesson-foundry-stable-id-');
  const outDir = join(dir, 'lessons');
  mkdirSync(outDir, { recursive: true });
  const findings = [
...
test('skill: a cluster with no defense file is still unbanked even though the id scan runs', (t) => {
  const dir = tmp(t, 'lesson-foundry-stable-id-');
  const outDir = join(dir, 'lessons');
  mkdirSync(outDir, { recursive: true });
  const findings = [
...
// ---------------------------------------------------------------------------
test('spec-gap: a legacy slug-only marker (no cluster-id) still credits the cluster', (t) => {
  const dir = tmp(t, 'lesson-foundry-stable-id-');
  const outDir = join(dir, 'lessons');
  mkdirSync(outDir, { recursive: true });
  const findings = [
...
test('skill: a legacy slug-named file (no id in content) still credits the cluster', (t) => {
  const dir = tmp(t, 'lesson-foundry-stable-id-');
  const outDir = join(dir, 'lessons');
  mkdirSync(outDir, { recursive: true });
  const findings = [
