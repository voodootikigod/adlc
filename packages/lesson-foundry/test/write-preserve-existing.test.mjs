...
test('AC1: a second --write leaves a hand-edited .lint.json byte-identical and reports it skipped', (t) => {
  const dir = tmp(t, 'lesson-foundry-write-preserve-');
...
test('AC2: --force restores the regenerated scaffold over a hand-edited file', (t) => {
  const dir = tmp(t, 'lesson-foundry-write-preserve-');
  writeLedger(dir, CLUSTER_ENTRIES);
  const outDir = join(dir, '.adlc', 'lessons');
  const first = runCli(['--write', '--out-dir', outDir], dir);
...
test('AC3: a fresh out-dir with plain --write (no --force) writes every planned file', (t) => {
  const dir = tmp(t, 'lesson-foundry-write-preserve-');
  writeLedger(dir, CLUSTER_ENTRIES);
  const outDir = join(dir, '.adlc', 'lessons');
...
test('AC4: --json --write on a mix of new and pre-existing files reports both written and skipped paths', (t) => {
  const dir = tmp(t, 'lesson-foundry-write-preserve-');
  writeLedger(dir, CLUSTER_ENTRIES);
  const outDir = join(dir, '.adlc', 'lessons');
  const first = runCli(['--write', '--out-dir', outDir], dir);
