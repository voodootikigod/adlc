...
// ---------------------------------------------------------------------------
test('CLI exit 0: empty ledger with no --gate', (t) => {
  const dir = tmp(t, 'lesson-foundry-cli-test-');
  writeLedger(dir, []);
  const { code } = runCli([], dir);
  assert.strictEqual(code, 0);
});

test('CLI exit 0: clusters present but --gate not supplied', (t) => {
  const dir = tmp(t, 'lesson-foundry-cli-test-');
  writeLedger(dir, [
...
test('CLI exit 0: --gate passes when all clusters are banked', (t) => {
  const dir = tmp(t, 'lesson-foundry-cli-test-');
  writeLedger(dir, [
    { ts: '2025-01-01', tool: 'test', file: 'a.mjs', line: 1, category: 'security', severity: 'high', desc: 'missing null check in database query' },
    { ts: '2025-01-02', tool: 'test', file: 'b.mjs', line: 2, category: 'security', severity: 'high', desc: 'missing null check in database query' },
...
// ---------------------------------------------------------------------------
test('CLI exit 1: --min receives non-numeric argument', (t) => {
  const dir = tmp(t, 'lesson-foundry-cli-test-');
  writeLedger(dir, []);
...
test('CLI exit 1: --min zero is rejected', (t) => {
  const dir = tmp(t, 'lesson-foundry-cli-test-');
  writeLedger(dir, []);
...
// ---------------------------------------------------------------------------
test('CLI exit 2: --gate fails when recurring cluster has no defense file', (t) => {
  const dir = tmp(t, 'lesson-foundry-cli-test-');
  writeLedger(dir, [
    { ts: '2025-01-01', tool: 'test', file: 'a.mjs', line: 1, category: 'security', severity: 'high', desc: 'missing null check in database query' },
    { ts: '2025-01-02', tool: 'test', file: 'b.mjs', line: 2, category: 'security', severity: 'high', desc: 'missing null check in database query' },
...
test('CLI exit 2: --gate details reported to stderr (not stdout)', (t) => {
  const dir = tmp(t, 'lesson-foundry-cli-test-');
  writeLedger(dir, [
...
// ---------------------------------------------------------------------------
test('CLI --json: stdout is valid JSON (no extra text)', (t) => {
  const dir = tmp(t, 'lesson-foundry-cli-test-');
  writeLedger(dir, [
    { ts: '2025-01-01', tool: 'test', file: 'a.mjs', line: 1, category: 'security', severity: 'high', desc: 'missing null check in database query' },
    { ts: '2025-01-02', tool: 'test', file: 'b.mjs', line: 2, category: 'security', severity: 'high', desc: 'missing null check in database query' },
...
test('CLI --json: empty ledger produces valid JSON with empty clusters', (t) => {
  const dir = tmp(t, 'lesson-foundry-cli-test-');
  writeLedger(dir, []);
  const { code, stdout } = runCli(['--json'], dir);
  assert.strictEqual(code, 0);
...
test('CLI --json: no "done" message or human text in stdout', (t) => {
  const dir = tmp(t, 'lesson-foundry-cli-test-');
  writeLedger(dir, []);
...
test('CLI --json --gate: exit 2 with valid JSON gate result when unbanked', (t) => {
  const dir = tmp(t, 'lesson-foundry-cli-test-');
  writeLedger(dir, [
    { ts: '2025-01-01', tool: 'test', file: 'a.mjs', line: 1, category: 'security', severity: 'high', desc: 'missing null check in database query' },
    { ts: '2025-01-02', tool: 'test', file: 'b.mjs', line: 2, category: 'security', severity: 'high', desc: 'missing null check in database query' },
...
// ---------------------------------------------------------------------------
test('CLI --write: repeated runs do not duplicate interrogation questions', (t) => {
  const dir = tmp(t, 'lesson-foundry-cli-test-');
  writeLedger(dir, [
    { ts: '2025-01-01', tool: 'test', file: 'a.mjs', line: 1, category: 'security', severity: 'high', desc: 'missing null check in database query' },
    { ts: '2025-01-02', tool: 'test', file: 'b.mjs', line: 2, category: 'security', severity: 'high', desc: 'missing null check in database query' },
...
test('CLI --write: a cluster GAINING a finding does not re-append its question (round-13)', (t) => {
...
test('CLI --write: an out-of-order merge introducing an OLDER member does not re-append (round-14)', (t) => {
...
test('CLI --write: a FUSED cluster whose uncovered members form a subcluster still gets a question (round-15)', (t) => {
...
test('CLI --json --gate: exit 0 with valid JSON when all banked', (t) => {
  const dir = tmp(t, 'lesson-foundry-cli-test-');
  writeLedger(dir, [
    { ts: '2025-01-01', tool: 'test', file: 'a.mjs', line: 1, category: 'security', severity: 'high', desc: 'missing null check in database query' },
    { ts: '2025-01-02', tool: 'test', file: 'b.mjs', line: 2, category: 'security', severity: 'high', desc: 'missing null check in database query' },
