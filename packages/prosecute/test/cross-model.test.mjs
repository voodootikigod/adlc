...
describe('recordCrossModelReview — fail-closed validation', () => {
  it('throws when provider equals authorProvider (not cross-model)', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    assert.throws(
  ...
  it('throws on missing/empty fields', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
  ...
  it('throws on an unknown verdict', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    assert.throws(
  ...
  it('refuses a same-provider record disguised by a WHITESPACE or CASE variant (normalized distinctness)', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
...
describe('hasCrossModelApprove — round-trip and binding', () => {
  it('a distinct-provider approve bound to the current revision satisfies the gate', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
  ...
  it('a stale-revision attestation does NOT satisfy the current revision', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
  ...
  it('a needs-attention verdict does NOT satisfy the gate', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
  ...
  it('an approve for a different ticket does NOT satisfy the gate', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T2', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'approve', dir });
    assert.equal(hasCrossModelApprove({ dir, ticket: 'T1', revision: 'rev-1', authorProvider: 'anthropic' }), false);
  });

  it('author anchoring: a prosecution author EQUAL to the reviewer provider does NOT pass', (t) => {
  ...
  it('author anchoring: an attestation recorded for a DIFFERENT author context does NOT pass', (t) => {
  ...
  it('empty manifest → false (fail-closed)', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    assert.equal(hasCrossModelApprove({ dir, ticket: 'T1', revision: 'rev-1', authorProvider: 'anthropic' }), false);
  });

  it('missing-arg guard: no ticket/revision/authorProvider → false (fail-closed public API)', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
  ...
  it('read side rejects a FORGED whitespace-variant reviewer that equals the prosecution author (injected, bypassing the writer)', (t) => {
  ...
  it('read side rejects a FORGED case-variant same-provider entry (both sides one provider, injected)', (t) => {
  ...
  it('normalized author-anchoring: a case/whitespace variant of a genuine distinct provider STILL passes', (t) => {
  ...
  it('the recorded entry carries provider, authorProvider, verdict and revision', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
...
describe('hasCrossModelApproveForRevision — #326 CI tier gate (ticket-agnostic)', () => {
  it('TRUE for a distinct-provider approve at the revision, regardless of the attestation ticket', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
  ...
  it('FALSE when the revision does not match (stale attestation cannot clear a fresh diff)', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-OLD', provider: 'openai', authorProvider: 'anthropic', verdict: 'approve', dir });
    assert.equal(hasCrossModelApproveForRevision({ dir, revision: 'rev-NEW', authorProvider: 'anthropic' }), false);
  });

  it('FALSE when the reviewer provider is not distinct from the run author', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
  ...
  it('FALSE for a needs-attention verdict and for missing args (fail closed)', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'needs-attention', dir });
  ...
  it('the record must be FOR this author (entry.authorProvider must equal the run author)', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
...
describe('cross-model TERMINAL revocation (T-MANIFEST-FOREST slice 2, spec §6)', () => {
  it('a needs-attention from the same provider REVOKES an approve for the same tuple (same revision)', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'approve', dir });
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'needs-attention', dir });
    assert.equal(hasCrossModelApproveForRevision({ dir, revision: 'rev-1', authorProvider: 'anthropic' }), false);
  ...
  it('TERMINAL, not "latest wins": a LATER approve after a needs-attention does NOT restore the gate', (t) => {
  ...
  it('a DIFFERENT provider standing approve still counts after another provider revokes', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'gemini', authorProvider: 'anthropic', verdict: 'approve', dir });
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'approve', dir });
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'needs-attention', dir });
...
describe('cross-model forest walk (T-MANIFEST-FOREST slice 2)', () => {
  it('a needs-attention recorded in a SEGMENT revokes an approve recorded in ROOT', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'approve', dir });
  ...
  it('an approve recorded in a SEGMENT satisfies the gate, and is revoked by a needs-attention in ROOT', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
  ...
  it('an UNSIGNED needs-attention in a fresh (never-signed) segment still revokes — regardless of signature state (spec §6)', (t) => {
  ...
  it('a segment revocation for a DIFFERENT ticket does not revoke a specific ticket-scoped approve', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'approve', dir });
    writeCrossModelSegment(dir, 'feat-01ARZ3NDEKTSV4RRFFQ69G5FAV.jsonl', {
  ...
  it('an UNTICKETED segment revocation blocks the ticket-agnostic gate but not a specific ticket-scoped approve', (t) => {
  ...
  it('a malformed/dangling segment fails the whole gate closed via manifestChainTrustworthy (not silently ignored)', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'approve', dir });
...
describe('cross-model manifest-tamper fail-closed (#326 Codex F2)', () => {
  it('a CORRUPTED later needs-attention line does NOT resurrect the revoked approve — fail closed', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'approve', dir });
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'needs-attention', dir });
  ...
  it('a TRUNCATED manifest (partial last line) fails closed rather than reading a stale approve', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'approve', dir });
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'needs-attention', dir });
    const lp = ledgerPath('manifest', dir);
...
describe('cross-model read-side entry-shape guards (#326)', () => {
  it('an entry whose data is not an object is ignored (no crash) — guards the null-data check', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
  ...
  it('an entry with an INVALID verdict is ignored, NOT treated as a revocation of a standing approve', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'approve', dir });
...
describe('signature verification (#326 — a PR author cannot forge an approve)', () => {
  it('an UNSIGNED approve (the forge: attacker appends data with no sig) is REJECTED', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
  ...
  it('an approve signed with a DIFFERENT key (attacker signs with their own) is REJECTED', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
  ...
  it('NO key at read time fails closed, even for a genuinely-signed attestation', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
...
describe('cross-model works on an unsigned legacy manifest (#326 — not inert, still forge-proof)', () => {
  it('a SIGNED approve IS trusted even when earlier manifest entries are UNSIGNED', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
  ...
  it('an UNSIGNED approve amid unsigned history is REJECTED — per-entry forge resistance survives chain-only', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    withoutKey(() => {
      record({ gate: 'build-gate', ticket: 'T0', rawData: JSON.stringify({ ok: true }), dir });
  ...
  it('a valid signed approve is fail-closed if a LATER entry is unsigned (signing lapsed after adoption)', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
...
describe('manifestChainBreakReason (#378)', () => {
  it('returns null for a legacy-unsigned-only manifest (lenient, not strict)', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    withoutKey(() => {
  ...
  it('returns "unsigned entry" for a signed-then-unsigned manifest', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'approve', dir });
    withoutKey(() => {
      record({ gate: 'rails', ticket: 'T0', rawData: JSON.stringify({ ok: true }), dir });
...
describe('hasCrossModelApprove input guard is fail-closed per clause (#326)', () => {
  it('a missing ticket fails closed even when a valid signed approve for the revision exists', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'approve', dir });
...
describe('cross-model rewrite-a-signed-revocation is caught (#354 F1)', () => {
  it('tampering a signed needs-attention (invalidating its sig) keeps the gate FAILED, not resurrected', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
...
describe('#365 F4/AC11 — identity form mismatch does not satisfy the gate', () => {
  it('an old-form (git-worktree:*) attestation does NOT satisfy a new-form (git-change:*) computation, and vice versa', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
  ...
  it('a new-form (git-change:*) attestation does NOT satisfy an old-form (git-worktree:*) computation', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    const repo = gitRepo(t);
    writeFileSync(join(repo.dir, 'src.txt'), 'base\n');
    repo.g('add', '-A'); repo.g('commit', '-qm', 'base');
...
describe('#365 F5/AC12 — a false base named in an attestation is never gate authority', () => {
  it('an attestation naming a fabricated base does not satisfy the gate for the REAL base, even with a matching change-set digest', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    const repo = gitRepo(t);
    writeFileSync(join(repo.dir, 'src.txt'), 'base\n');
    repo.g('add', '-A'); repo.g('commit', '-qm', 'base');
...
describe('truncation anti-rollback anchor (#355, #354 F1 follow-up)', () => {
  it('AC1: without observedEntries, truncating a signed revocation wrongly PASSES the gate; with it, the gate correctly FAILS', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    const storeDir = tmp(t, 'adlc-cross-model-');
  ...
  it('AC2: omitting observedEntries reproduces today\'s (#354) behavior exactly — no regression', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'approve', dir });
  ...
  it('honest residual limit: a revocation never observed by a trusted run is invisible (documented, not a regression)', (t) => {
    const dir = tmp(t, 'adlc-cross-model-');
    recordCrossModelReview({ ticket: 'T1', revision: 'rev-1', provider: 'openai', authorProvider: 'anthropic', verdict: 'approve', dir });
  ...
  it('round-2 codex finding: an observed revocation for a DIFFERENT authorProvider at the same revision does NOT falsely block (author-scoped, not revision-only)', (t) => {
    ...
    const prOneDir = tmp(t, 'adlc-cross-model-');
    const prTwoDir = tmp(t, 'adlc-cross-model-');
