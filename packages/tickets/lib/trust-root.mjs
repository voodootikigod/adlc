 ...
 * out.
 *
 * KNOWN BOUNDARY — a repo that used rails, had NO hook override ever recorded, and
 * removed its last railed ticket with the PRE-change writer. Only then is every
 * source silent: the ticket set, the archive, this contract's own audit entries, and
 * the hook's rails-bypass entries. The evidence does not exist to read, and closing
 ...
 * can synthesise after the fact. In practice a repo that enforced rails through the
 * PreToolUse hook has rails-bypass entries and is covered; every removal from this
 * change forward leaves a marker regardless.
 *
