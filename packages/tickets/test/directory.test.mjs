...
test('directory and legacy stores produce the same immutable logical snapshot', (t) => {
  const dir = tmp(t, 'adlc-tickets-directory-');
...
test('closed layout rejects extra files, nested directories, symlinks, filename mismatch, dangling edges, and cycles', (t) => {
  const cases = [
...
test('store roots and legacy files cannot be symlinks', (t) => {
  const dir = tmp(t, 'adlc-tickets-directory-');
...
test('a symlinked store parent cannot redirect active storage outside the repository', (t) => {
  const dir = tmp(t, 'adlc-tickets-directory-');
  const external = tmp(t, 'adlc-tickets-directory-');
