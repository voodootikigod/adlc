...
import { mkdirSync, writeFileSync, readFileSync, readdirSync, existsSync, rmSync, symlinkSync, renameSync, chmodSync } from 'node:fs';
...
function repo(t, branch = 'feat/ticket-evidence') {
  const { dir: root, g } = gitRepo(t, { prefix: 'adlc-ticket-segments-', branch });
...
describe('isSegmentedRepo (mirrors @adlc/gate-manifest)', () => {
  ...
  it('a symlinked marker is never followed — treated as absent', (t) => {
    const { dir } = repo(t);
    const outsideDir = tmp(t, 'outside-marker-');
    const outsideTarget = join(outsideDir, 'outside-marker.txt');
  ...
  it('the marker read cap boundary is exact: 4096 bytes is refused, 4095 is accepted', (t) => {
    const { root, dir } = repo(t);
    mkdirSync(join(dir, 'manifest.d'), { recursive: true });
...
describe('recordTicketEvidence routes to the segment writer once segmented', () => {
  it('a non-segmented repo is unaffected — still appends to root', (t) => {
    const { root, dir } = repo(t);
  ...
  it('a segmented, root-less repo writes its first entry as a new, anchor:null segment — never touching root', (t) => {
    const { root, dir } = repo(t);
    activate(dir);
    const entry = recordTicketEvidence(root, baseEvidence());
    assert.equal(entry.seq, 1);
  ...
  it('recognizes a segment whose slug contains digits 2-9 (full character-class range, not just 0-1)', (t) => {
    const { root, dir } = repo(t, 'feat/t789-full-digit-range');
    activate(dir);
    const first = recordTicketEvidence(root, baseEvidence({ transactionId: 'tx-1' }));
  ...
  it('a second evidence append on the same branch continues the same segment', (t) => {
    const { root, dir } = repo(t);
    activate(dir);
  ...
  it('idempotency: a matching retry returns the existing segment entry without appending a duplicate', (t) => {
    const { root, dir } = repo(t);
    activate(dir);
  ...
  it('idempotency conflict: same transaction/action with DIFFERENT evidence is refused, in a segment', (t) => {
    const { root, dir } = repo(t);
    activate(dir);
  ...
  it('entries are signed at v2 when a key is present (anchor coverage)', (t) => {
    const { root, dir } = repo(t);
    activate(dir);
  ...
  it('refuses to append when a DIFFERENT segment (not the target) has a broken chain', (t) => {
    const { root, dir } = repo(t, 'feat/branch-a');
    activate(dir);
  ...
  it('idempotency scan is forest-wide: a prior root entry (pre-migration) is still found once segmented', (t) => {
    const { root, dir } = repo(t);
  ...
  it('refuses to trust a forged, correctly-chained-but-UNSIGNED entry once this chain\'s signed era has begun', (t) => {
    const { root, dir } = repo(t);
    activate(dir);
  ...
  it('refuses to trust a match found in a wholly-UNSIGNED forged segment, even though its chain passes on its own', (t) => {
    const { root, dir } = repo(t);
    activate(dir);
  ...
  it('rejects a nested directory under manifest.d/ — forestChainsIntact must agree with gate-manifest verify()', (t) => {
    const { root, dir } = repo(t);
    activate(dir);
  ...
  it('a real segment disguised with a .lock suffix is NOT silently hidden — its content does not match a genuine lock', (t) => {
    const { root, dir } = repo(t);
    activate(dir);
...
describe('recoverOpenSegment (lineage-durability finding)', () => {
  ...
  it('AC1: finds a committed segment on a FRESH CLONE, which never has a local .lineage token', (t) => {
    const { root, dir } = repo(t, 'feat/clone-recovery');
    activate(dir);
    recordTicketEvidence(root, baseEvidence());
    const resolved = resolveOpenSegment(dir, { cwd: root });
    ...
    const clonedRoot = tmp(t, 'adlc-ticket-segments-clone-');
  ...
  it('AC3: refuses to guess when more than one committed segment declares this branch as its own', (t) => {
    const { root, dir } = repo(t, 'feat/ambiguous');
    activate(dir);
    recordTicketEvidence(root, baseEvidence());
  ...
  it('a segment that exists but cannot be opened (permission denied) makes recovery refuse, never silently excludes it', { skip: process.platform === 'win32' }, (t) => {
    ...
    t.after(() => { try { chmodSync(unreadablePath, 0o644); } catch {} });
    rmSync(lineagePath(dir), { force: true });
  ...
  // recoverOpenSegment must not scan only discoverSegments(dir).valid while
  // silently ignoring .invalid — a real segment renamed to a bad-grammar name,
  // replaced with a symlink, or otherwise turned into a non-conforming filesystem
  // object would become indistinguishable from "never existed".
  // forestChainsIntact (the write-time precondition) refuses on ANY invalid
  // object anywhere in the forest; recovery enforces the equivalent behavior.
  ...
  // state the way an empty ROOT is — gate-manifest's own verifyChain treats an empty
  // segment as INVALID ("has no first entry to carry the required anchor"), since
  // every real segment's mint atomically writes its anchor-carrying first entry. A
  // zero-byte segment can only mean a crash between file creation and first append,
  // or truncation/tampering — recovery must refuse it, not silently skip it as a
  // non-candidate the way a merely-absent file would be.
  ...
  it('AC12: a fresh clone whose FIRST action is a real WRITE extends the branch\'s own committed segment, and the pre-clone evidence stays visible to a reader afterwards', (t) => {
    ...
    const { root, dir } = repo(t, 'feat/clone-write-first');
    activate(dir);
    ...
    const clonedRoot = tmp(t, 'adlc-ticket-segments-clone-write-');
  ...
  it('AC12: refuses (ambiguous) rather than mint when recovery finds more than one candidate segment for this branch', (t) => {
    const KEY = 'ambiguity-key';
    const { root, dir } = repo(t, 'feat/ambiguous-write');
    activate(dir);
  ...
  it('AC13: a KEYLESS fresh clone whose first action is a write REFUSES — never extends, never shadow-mints', (t) => {
    const { root, dir } = repo(t, 'feat/keyless-clone');
    activate(dir);
    ...
    const clonedRoot = tmp(t, 'adlc-ticket-keyless-clone-');
  ...
  it('AC13: a KEYED writer refuses a candidate whose branch claim rides a v1 signature', (t) => {
    const KEY = 'v1-forgery-key';
    const { root, dir } = repo(t, 'feat/v1-forged');
    activate(dir);
    ...
    const clonedRoot = tmp(t, 'adlc-ticket-v1-forged-');
  ...
  it('AC13: a KEYED writer refuses an unauthenticatable single candidate — no extend, no duplicate mint', (t) => {
    const { root, dir } = repo(t, 'feat/unauth-candidate');
    activate(dir);
    ...
    const clonedRoot = tmp(t, 'adlc-ticket-unauth-clone-');
  ...
  it('AC15: a keyed-mode marker refuses a keyless ticket-evidence write before touching anything', (t) => {
    const { root, dir } = repo(t, 'feat/keyed-forest');
    mkdirSync(join(dir, 'manifest.d'), { recursive: true });
  ...
  it('AC14: minting refuses when the branch-derived segment filename is gitignored — before any evidence is recorded', (t) => {
...
describe('readOwnChains: allowRecovery is opt-in, defaults to strict token-only (lineage-durability finding)', () => {
  it('default (no allowRecovery): does not recover — returns root-only once the token is lost, even with a real committed segment AND a key that could verify it', (t) => {
    const { root, dir } = repo(t);
  ...
  it('allowRecovery: true with NO key REFUSES when a candidate segment exists — never silently treats it as absent', (t) => {
    const { root, dir } = repo(t);
    activate(dir);
    recordTicketEvidence(root, baseEvidence());
    rmSync(lineagePath(dir), { force: true });
  ...
  it('allowRecovery: true with NO key returns root-only when genuinely no candidate segment exists', (t) => {
    const { root, dir } = repo(t);
  ...
  it('allowRecovery: true on detached HEAD REFUSES when committed segments exist — never silently treats them as absent', (t) => {
    const { root, dir } = repo(t, 'feat/detach-me');
  ...
  it('allowRecovery: true on detached HEAD returns root-only when genuinely no segments exist anywhere', (t) => {
    const { root, dir } = repo(t);
  ...
  it('allowRecovery: true WITH a key opts into the exact branch-field recovery scan, trusting only signature-verified entries', (t) => {
    const { root, dir } = repo(t);
  ...
  it('allowRecovery: true WITH a key refuses the whole read when a signed segment is followed by an unsigned entry, even with an exact branch match', (t) => {
    const { root, dir } = repo(t, 'feat/forged-branch');
    const KEY = 'recovery-trust-key';
    activate(dir);
  ...
  it('allowRecovery: true refuses the whole read when a segment\'s LATEST signed entry is tampered, never resurrecting an earlier stale verdict', (t) => {
    const { root, dir } = repo(t, 'feat/resurrection');
  ...
  it('allowRecovery: true refuses an ENTIRELY unsigned recovered segment, even though its hash chain is perfectly consistent', (t) => {
    const { root, dir } = repo(t, 'feat/wholly-unsigned');
  ...
  it('allowRecovery: true never returns an unsigned forged entry, even once a later, unrelated entry in the same chain is genuinely signed', (t) => {
    const { root, dir } = repo(t, 'feat/laundered-prefix');
  ...
  it('a valid token (peeked, not recovered) still refuses a signed-then-unsigned segment when a key is available', (t) => {
    const { root, dir } = repo(t, 'feat/peeked-tamper');
  ...
  it('root itself still refuses a signed-then-unsigned chain when a key is available, independent of segmentation', (t) => {
    const { root, dir } = repo(t);
  ...
  it('root itself refuses when ENTIRELY unsigned but a key is available, even with a perfectly consistent hash chain', (t) => {
    const { root, dir } = repo(t);
  ...
  it('root with ZERO entries is safe even with a key available — empty is not "unsigned"', (t) => {
    const { root, dir } = repo(t);
  ...
  it('a token-resolved (peeked) segment that is empty on disk refuses, with or without a key', (t) => {
    const { root, dir } = repo(t, 'feat/peeked-empty');
    activate(dir);
...
describe('generateSegmentUlid (spec §4.2)', () => {
  ...
  it('rejects entropy that is not exactly 10 bytes', (t) => {
  ...
  it('rejects an out-of-range timestamp', (t) => {
