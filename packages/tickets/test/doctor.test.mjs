...
test('doctor is read-only and reports active/archive/runtime health', (t) => {
  const root = tmp(t, 'adlc-tickets-doctor-');
...
function doctorWith(t, pointer, { raw = false } = {}) {
  const root = tmp(t, 'adlc-doctor-pointer-');
  ...
  return { report, check: report.checks.find((c) => c.name === 'current-ticket'), snapshot };
}

test('doctor current-ticket: absent pointer is healthy, not broken', (t) => {
  const d = doctorWith(t, undefined);
  assert.equal(d.check.ok, true);
  assert.equal(d.check.present, false);
});

test('doctor current-ticket: a canonical pointer is healthy and names the ticket', (t) => {
  const d = doctorWith(t, (s) => ({ id: 'A', ticketHash: s.ticketHashes.A }));
  assert.equal(d.check.ok, true);
  assert.equal(d.check.id, 'A');
  assert.equal(d.check.deprecatedAlias, undefined);
});

test('doctor current-ticket: a deprecated alias resolves but is reported', (t) => {
  ...
  const d = doctorWith(t, (s) => ({ ticketId: 'A', ticketHash: s.ticketHashes.A }));
  assert.equal(d.check.ok, true);
  assert.equal(d.check.id, 'A');
  assert.equal(d.check.deprecatedAlias, 'ticketId');
});

test('doctor current-ticket: a stale hash is reported, not called healthy', (t) => {
  const d = doctorWith(t, { id: 'A', ticketHash: 'f'.repeat(64) });
...
test('doctor current-ticket: a pointer naming an absent ticket is reported', (t) => {
  const d = doctorWith(t, { id: 'GONE', ticketHash: 'f'.repeat(64) });
  assert.equal(d.check.ok, false);
  assert.equal(d.check.code, 'ACTIVE_TICKET_MISSING');
});

test('doctor current-ticket: an unrecognized id key is reported, not treated as absent', (t) => {
  // The fail-open shape. Doctor must not shrug at a pointer the gates will deny.
  const d = doctorWith(t, { tickett: 'A' });
  assert.equal(d.check.ok, false);
  assert.equal(d.check.code, 'INVALID_CURRENT_TICKET');
  assert.equal(d.check.present, true);
});

test('doctor current-ticket: an unparseable pointer is reported', (t) => {
  const d = doctorWith(t, '{not json', { raw: true });
  assert.equal(d.check.ok, false);
  assert.equal(d.check.code, 'INVALID_CURRENT_TICKET');
});

test('doctor current-ticket: a pointer pinning no ticketHash is reported (strict, as 2.0 will enforce)', (t) => {
  const d = doctorWith(t, { id: 'A' });
  assert.equal(d.check.ok, false);
...
function storeWithEvidence(t, extra = []) {
  const root = tmp(t, 'adlc-doctor-bind-');
  ...
  for (const extraTicket of extra) service.apply(service.planCreate(extraTicket));
...
test('doctor storehash-manifest-bind: a clean store (unchanged since the last evidence) passes and binds', (t) => {
  const { root, store } = storeWithEvidence(t);
  const report = doctorTicketStore(store, { root });
...
test('doctor storehash-manifest-bind: WITHOUT a key, a coordinated shard+final-entry edit hides drift — but is flagged UNAUTHENTICATED', (t) => {
...
test('doctor storehash-manifest-bind: a hand-edited shard surfaces as drift, deliberately NOT adjudicated as tamper', (t) => {
  const { root, store } = storeWithEvidence(t);
...
test('doctor storehash-manifest-bind: a legit later update of an EVIDENCED ticket is NOT flagged as tamper (no false positive)', (t) => {
  ...
  const { root, store, service } = storeWithEvidence(t);
...
test('doctor storehash-manifest-bind: a legitimate unevidenced op (create) is reported as drift, not failed', (t) => {
  const { root, store, service } = storeWithEvidence(t);
...
test('doctor storehash-manifest-bind: a forged / chain-broken manifest is NOT trusted (no arbitrary checkpoint)', (t) => {
  const { root, store } = storeWithEvidence(t);
...
test('doctor storehash-manifest-bind: a manifest line that is not valid JSON FAILS the integrity check', (t) => {
  const { root, store } = storeWithEvidence(t);
...
test('doctor storehash-manifest-bind: an honest unsigned legacy prefix followed by signed entries verifies', (t) => {
  const { root, store } = storeWithEvidence(t);
...
test('doctor storehash-manifest-bind: an unsigned entry AFTER a signed one is still rejected', (t) => {
  const { root, store } = storeWithEvidence(t);
...
test('doctor storehash-manifest-bind: a PRESENT-but-invalid signature inside the prefix is still rejected', (t) => {
  const { root, store } = storeWithEvidence(t);
...
test('doctor storehash-manifest-bind: an UNSIGNED entry may not supply the bound checkpoint when a key is set', (t) => {
  const { root, store } = storeWithEvidence(t);
  const live = store.load().hash;
...
test('doctor storehash-manifest-bind: a one-character signature is tampering, not a legacy entry', (t) => {
  const { root, store } = storeWithEvidence(t);
...
test('doctor storehash-manifest-bind: the failure reason names the OFFENDING line, 1-indexed', (t) => {
  const { root, store } = storeWithEvidence(t);
...
test('doctor storehash-manifest-bind: an entry carrying no `data` field at all is walked, not thrown on', (t) => {
  const { root, store } = storeWithEvidence(t);
  const live = store.load().hash;
...
test('doctor storehash-manifest-bind: with no checkpoint recorded at all, the reason does not blame signing', (t) => {
  const { root, store } = storeWithEvidence(t);
...
test('doctor storehash-manifest-bind: with a key set, a validly-signed manifest verifies (signaturesVerified)', (t) => {
  const { root, store } = storeWithEvidence(t);
  const live = store.load().hash;
...
test('doctor storehash-manifest-bind: an in-place edit of the FINAL entry storeHash fails signature verification', (t) => {
  const { root, store } = storeWithEvidence(t);
...
test('doctor storehash-manifest-bind: with NO key configured, it reports signatures were not verified', (t) => {
  const { root, store } = storeWithEvidence(t);
  const prevKey = process.env.ADLC_MANIFEST_KEY;
  delete process.env.ADLC_MANIFEST_KEY;
  t.after(() => {
...
test('doctor storehash-manifest-bind: a store with no recorded evidence yet is inert (not a failure)', (t) => {
  const root = tmp(t, 'adlc-doctor-noevidence-');
...
function gitStoreWithSegmentedEvidence(t, { key = null } = {}) {
  const root = tmp(t, 'adlc-doctor-segment-');
...
test('doctor storehash-manifest-bind: binds to evidence recorded in a segment (segmented repo), not just root', (t) => {
  const { root, store } = gitStoreWithSegmentedEvidence(t);
  const report = doctorTicketStore(store, { root });
  const check = bindCheck(report);
...
test('doctor storehash-manifest-bind: recovers a real checkpoint across a lost .lineage token (fresh-clone/branch-switch case)', (t) => {
  const { root, store } = gitStoreWithSegmentedEvidence(t);
...
test('doctor storehash-manifest-bind: a genuinely ambiguous recovery fails closed as SEGMENT_AMBIGUOUS, never guesses', (t) => {
  const { root, store } = gitStoreWithSegmentedEvidence(t);
...
test('doctor storehash-manifest-bind: a signed segment checkpoint is authenticated the same as a signed root one', (t) => {
  // The key is an EXPLICIT parameter now — env manipulation is inert by design
  // (spec Layer 2, P1): both evidence recording and doctor consult only what
  // they are handed.
  const { root, store } = gitStoreWithSegmentedEvidence(t, { key: 'doctor-segment-test-key' });
...
test('doctor storehash-manifest-bind: a corrupted segment chain fails closed, not silently ignored', (t) => {
  const { root, store } = gitStoreWithSegmentedEvidence(t);
  const segDir = join(root, '.adlc', 'manifest.d');
  const segName = readdirSync(segDir).find((n) => n.endsWith('.jsonl'));
...
test('doctor storehash-manifest-bind: never mints a segment or writes .lineage (read-only)', (t) => {
  const root = tmp(t, 'adlc-doctor-segment-nomint-');
  const g = (...args) => execFileSync('git', args, { cwd: root, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] });
...
test('doctor current-ticket: reports that it could not validate when the store is unreadable', (t) => {
  ...
  const root = tmp(t, 'adlc-doctor-nostore-');
 ...
 */
function gitStoreWithAnchoredSegment(t) {
  const root = tmp(t, 'adlc-doctor-forest-');
  const g = (...args) => execFileSync('git', args, { cwd: root, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] });
...
test('doctor manifest-forest: a healthy forest reports the check ok', (t) => {
  const { root, store, segment } = gitStoreWithAnchoredSegment(t);
...
test('doctor manifest-forest: a repo that never segmented is inert, not a failure', (t) => {
...
test('doctor manifest-forest: reports an anchor whose root line no longer exists', (t) => {
  const { root, store } = gitStoreWithAnchoredSegment(t);
...
test('doctor manifest-forest: reports an anchor naming a segment that is not in the forest', (t) => {
  const { root, store, segment } = gitStoreWithAnchoredSegment(t);
...
test('doctor manifest-forest: reports a .lineage token whose segment no longer exists', (t) => {
  const { root, store, segment } = gitStoreWithAnchoredSegment(t);
...
test('doctor manifest-forest: reports a .lineage token whose cached ULID no longer matches the segment file', (t) => {
  const { root, store, segment } = gitStoreWithAnchoredSegment(t);
...
test('doctor manifest-forest: reports both faults and repairs neither — every byte under .adlc/ is unchanged', (t) => {
  const { root, store, segment } = gitStoreWithAnchoredSegment(t);
