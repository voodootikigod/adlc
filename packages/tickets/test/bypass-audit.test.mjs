...
function repo(t, tickets) {
  const root = tmp(t, 'bypass-audit-');
...
test('EMPIRICAL PROBE (the bug): a keyless bypassed create against a frozen trust root no longer mutates in silence', (t) => {
  const root = repo(t, [ticket('T-RAILED', { rails: ['src/**'] })]);
...
test('AC1 rails frozen + key present: create --write appends exactly ONE signed ticket-mutation entry bound to the new store hash', (t) => {
  const root = repo(t, [ticket('T-RAILED', { rails: ['src/**'] })]);
  const before = storeHashOf(root);
...
test('AC2 rails frozen + NO key: create --write refuses before writing, names the key, and leaves no pending transaction', (t) => {
  const root = repo(t, [ticket('T-RAILED', { rails: ['src/**'] })]);
  const before = storeHashOf(root);
  const result = runTicket(root, ['create', '--input', inputFile(root, newTicket('T-NEW')), '--write', '--json']);
...
test('AC2 opt-out: --allow-unsigned writes the mutation and an UNSIGNED audit entry, and warns what that costs', (t) => {
  const root = repo(t, [ticket('T-RAILED', { rails: ['src/**'] })]);
...
test('the unsigned warning reaches the store ceremonies too, not just ticket mutations', (t) => {
  const root = tmp(t, 'bypass-audit-ceremony-warn-');
...
test('archive warns exactly once about an unsigned audit entry, like every other mutation', (t) => {
  const root = repo(t, [ticket('T-RAILED', { rails: ['src/**'] }), ticket('T-OLD')]);
...
test('AC2 --allow-unsigned is inert when a key IS present: the entry is still signed, and nothing warns', (t) => {
  const root = repo(t, [ticket('T-RAILED', { rails: ['src/**'] })]);
...
test('--allow-unsigned takes no value: flag order does not change the outcome', (t) => {
  const root = repo(t, [ticket('T-RAILED', { rails: ['src/**'] })]);
...
test('TicketService defaults allowUnsigned OFF: omitting it refuses a keyless write to a trust root', (t) => {
  const root = tmp(t, 'bypass-audit-default-');
...
test('storeDeclaresRails fails CLOSED on anything it cannot read as a definite no-rails', (t) => {
...
test('AC3 no rails declared anywhere: create --write records nothing and succeeds with no key', (t) => {
  const root = repo(t, [ticket('A')]); // rails: []
  const result = runTicket(root, ['create', '--input', inputFile(root, newTicket('T-NEW')), '--write', '--json']);
  assert.equal(result.status, 0, result.stderr);
...
test('AC3 boundary: a COMPLETED ticket\'s rails still make the store a frozen trust root', (t) => {
  const root = repo(t, [ticket('T-DONE', { rails: ['src/**'], completed: true })]);
  const result = runTicket(root, ['create', '--input', inputFile(root, newTicket('T-NEW')), '--write', '--json']);
  assert.notEqual(result.status, 0, 'expired rails do not thaw the trust root');
  assert.equal(manifestEntries(root).length, 0);
});

test('AC4 update (no sensitive change) under a frozen trust root records one ticket-mutation entry', (t) => {
  const root = repo(t, [ticket('T-RAILED', { rails: ['src/**'] }), ticket('T-EDIT')]);
  const before = storeHashOf(root);
...
test('AC4 complete under a frozen trust root records one entry carrying the bypass audit', (t) => {
  const root = repo(t, [ticket('T-RAILED', { rails: ['src/**'] }), ticket('T-DOING')]);
  const before = storeHashOf(root);
...
test('AC4 complete under a frozen trust root REFUSES with no key, exactly like create', (t) => {
  const root = repo(t, [ticket('T-RAILED', { rails: ['src/**'] }), ticket('T-DOING')]);
  const before = storeHashOf(root);
...
test('AC4 update --authorize (rail narrowing) keeps its existing entry — one mutation, NOT two', (t) => {
  const root = repo(t, [ticket('T-RAILED', { rails: ['src/**', 'lib/**'] })]);
  const before = storeHashOf(root);
...
test('a NO-OP transaction against a frozen trust root needs no key and records nothing', (t) => {
  const root = tmp(t, 'bypass-audit-noop-');
...
test('an UNRAILED store keeps its existing evidence shape byte for byte — the change is scoped to frozen trust roots', (t) => {
  const root = repo(t, [ticket('T-DOING')]);
  const result = runTicket(root, ['complete', 'T-DOING', '--write', '--json'], { ADLC_MANIFEST_KEY: KEY });
  assert.equal(result.status, 0, result.stderr);
  const [entry] = manifestEntries(root);
...
test('archive REFUSES a keyless write to a trust root leaving NO filesystem trace — not even the archive store', (t) => {
  const root = tmp(t, 'bypass-audit-archive-');
...
test('archive with a key still works and records its own gate, now carrying the bypass audit', (t) => {
  const root = tmp(t, 'bypass-audit-archive-ok-');
  const path = writeDirectory(root, [ticket('T-RAILED', { rails: ['src/**'] }), ticket('T-OLD')]);
  const store = new DirectoryTicketStore(path);
  const before = store.load().hash;
  archiveTicket(store, join(root, '.adlc/ticket-archive'), 'T-OLD', {
...
test('a legacy-store migration REFUSES without a key when the store it rewrites declares rails', (t) => {
  const root = tmp(t, 'bypass-audit-migrate-');
  writeLegacy(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
  assert.throws(
...
test('an UNRAILED legacy store still migrates with no key — migration is not ceremony by itself', (t) => {
  const root = tmp(t, 'bypass-audit-migrate-unrailed-');
...
test('recovery REFUSES to finish a bypassed mutation without a key, and leaves the transaction pending', (t) => {
  const root = tmp(t, 'bypass-audit-recover-');
  const path = writeDirectory(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
  const store = new DirectoryTicketStore(path);
  const before = store.load().hash;
...
test('recovering the transaction that introduced the FIRST rail needs no key', (t) => {
  for (const direction of ['complete', 'rollback']) {
    const root = tmp(t, 'bypass-audit-first-rail-');
...
test('recovering an UPDATE that introduced the first rail needs no key either', (t) => {
  for (const direction of ['complete', 'rollback']) {
    const root = tmp(t, 'bypass-audit-first-rail-update-');
...
test('recovering a restore of the last railed ARCHIVED ticket still requires a key', (t) => {
  const root = tmp(t, 'bypass-audit-restore-');
...
test('a backup that does not match its recorded hash is not believed', (t) => {
  const root = tmp(t, 'bypass-audit-swapped-backup-');
  const path = writeDirectory(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
  const store = new DirectoryTicketStore(path);
...
// already handles.
test('recovering a LEGACY write that introduced the first rail needs no key', (t) => {
  for (const direction of ['complete', 'rollback']) {
    const root = tmp(t, 'bypass-audit-legacy-first-rail-');
...
test('a legacy journal with no bypassAudit field falls back to the store, and fails closed', (t) => {
  const root = tmp(t, 'bypass-audit-legacy-journal-');
  const path = writeDirectory(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
  const store = new DirectoryTicketStore(path);
  assert.throws(() => applyDirectoryTransaction(store, [...store.load().mutableTickets(), ticket('T-NEW')], {
    root, operation: 'create', ticketId: 'T-NEW', key: KEY,
...
test('recoverMigration defaults allowUnsigned OFF: finishing an interrupted trust-root migration needs a key', (t) => {
  const root = tmp(t, 'bypass-audit-migrate-recover-');
  writeLegacy(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
...
test('recovery ignores a journal that claims NO audit is owed — the store decides, the journal may only add', (t) => {
  const root = tmp(t, 'bypass-audit-tampered-journal-');
  const path = writeDirectory(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
  const store = new DirectoryTicketStore(path);
  assert.throws(() => applyDirectoryTransaction(store, [...store.load().mutableTickets(), ticket('T-NEW')], {
    root, operation: 'create', ticketId: 'T-NEW', key: KEY,
...
test('the exported trust-root guard fails CLOSED on an omitted or invalid key, per the key contract', (t) => {
  const root = tmp(t, 'bypass-audit-guard-');
  const railed = [ticket('T-RAILED', { rails: ['src/**'] })];
...
test('the interactive migration prompt carries the resolved key through to the migration', async (t) => {
  const root = tmp(t, 'bypass-audit-prompt-');
  writeLegacy(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
  const store = new LegacyTicketStore(join(root, '.adlc/tickets.json'));
  const seen = [];
...
test('a migration of a trust root records its own gate carrying the audit fields', (t) => {
  const root = tmp(t, 'bypass-audit-migrate-fields-');
  writeLegacy(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
...
test('an UNRAILED migration keeps its evidence shape unchanged — no audit fields', (t) => {
  const root = tmp(t, 'bypass-audit-migrate-plain-');
  writeLegacy(root, [ticket('A')]);
  migrateLegacyStore(root, { write: true, yes: true, requireClean: false, key: KEY });
  const [entry] = manifestEntries(root);
...
test('a recovery audit binds the hash the store ACTUALLY held when recovery started', (t) => {
  const root = tmp(t, 'bypass-audit-recover-hash-');
  const path = writeDirectory(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
  const store = new DirectoryTicketStore(path);
...
test('archiving the LAST railed ticket does not thaw the trust root — the archive still declares rails', (t) => {
  const root = tmp(t, 'bypass-audit-thaw-');
  const path = writeDirectory(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
  const store = new DirectoryTicketStore(path);
...
test('an archive entry that cannot be read as a ticket fails CLOSED, whatever it is named', (t) => {
  ...
  for (const [label, plant] of [
    ...
    const root = tmp(t, 'bypass-audit-opaque-archive-');
...
test('discarding the LAST railed ticket does not thaw the trust root — the manifest remembers', (t) => {
  const root = tmp(t, 'bypass-audit-discard-thaw-');
  const path = writeDirectory(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
  const store = new DirectoryTicketStore(path);
...
test('emptying the last rails array does not thaw the trust root either', (t) => {
  const root = tmp(t, 'bypass-audit-unrail-thaw-');
  const path = writeDirectory(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
  const store = new DirectoryTicketStore(path);
  const service = new TicketService(store, { root, key: KEY });
...
test('a symlink where trust-root evidence belongs fails CLOSED, dangling or not', (t) => {
  const active = [ticket('A')];
  for (const [label, plant] of [
    ...
    const root = tmp(t, 'bypass-audit-symlink-');
...
test('an unreadable evidence path and a corrupted bypass marker both fail CLOSED', (t) => {
  const active = [ticket('A')];
  ...
  const notADirectory = tmp(t, 'bypass-audit-notadir-');
  ...
  const unreadable = tmp(t, 'bypass-audit-unreadable-');
  ...
  if (deniesReads) {
    t.after(() => { try { chmodSync(adlc, 0o755); } catch {} });
    assert.throws(
  ...
  const corrupted = tmp(t, 'bypass-audit-corrupt-marker-');
...
test("the rail hook's own rails-bypass entries keep the trust root frozen", (t) => {
  const root = tmp(t, 'bypass-audit-hook-marker-');
...
test('an EMPTY archive next to a rails-free store is still not a trust root', (t) => {
  const root = tmp(t, 'bypass-audit-empty-archive-');
...
test('the LEGACY archive file counts too', (t) => {
  const root = tmp(t, 'bypass-audit-legacy-archive-');
  writeDirectory(root, [ticket('A')]);
  writeFileSync(
...
test('recovery of a transaction that removed the last rail is still an audited trust-root write', (t) => {
  const root = tmp(t, 'bypass-audit-lastrail-');
...
test('no ticket store class can be written through UNAUDITED', (t) => {
  const root = tmp(t, 'bypass-audit-readonly-');
  writeLegacy(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
...
test('LegacyTicketStore.write still works for 1.x callers, and is now audited', (t) => {
  const plain = tmp(t, 'bypass-audit-legacy-write-');
  writeLegacy(plain, [ticket('A')]);
  const plainStore = new LegacyTicketStore(join(plain, '.adlc/tickets.json'));
  const after = plainStore.write([ticket('A'), ticket('B')]); // the 1.x one-argument call
  ...
  const railed = tmp(t, 'bypass-audit-legacy-write-railed-');
  writeLegacy(railed, [ticket('T-RAILED', { rails: ['src/**'] })]);
  const railedStore = new LegacyTicketStore(join(railed, '.adlc/tickets.json'));
  const before = readFileSync(join(railed, '.adlc/tickets.json'), 'utf8');
  assert.throws(
    () => railedStore.write([ticket('T-RAILED', { rails: ['src/**'] }), ticket('B')]),
    (error) => error.code === 'MANIFEST_KEY_REQUIRED',
  ...
  railedStore.write([ticket('T-RAILED', { rails: ['src/**'] }), ticket('B')], { key: KEY });
...
test('LegacyTicketStore.write refuses to GUESS the repository root for a store outside the canonical layout', (t) => {
  const repoRoot = tmp(t, 'bypass-audit-external-repo-');
  const external = tmp(t, 'bypass-audit-external-store-');
...
test('store export REFUSES to write onto a ticket store, whichever store path it names', (t) => {
  const root = tmp(t, 'bypass-audit-export-');
  const path = writeDirectory(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
  const store = new DirectoryTicketStore(path);
  for (const target of [
    '.adlc/tickets.json',
...
test('store export refuses a path that only reaches a ticket store through a symlink', (t) => {
  const root = tmp(t, 'bypass-audit-export-link-');
  const path = writeDirectory(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
  const store = new DirectoryTicketStore(path);
...
test('store export refuses a destination inside a DIFFERENT directory store, not just the source', (t) => {
  const root = tmp(t, 'bypass-audit-export-other-store-');
  const otherRoot = tmp(t, 'bypass-audit-export-store-b-');
...
test('recovering a migration that already recorded its apply evidence completes instead of conflicting', (t) => {
  const root = tmp(t, 'bypass-audit-replay-');
  writeLegacy(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
...
test('migration rollback still requires a key when only the ARCHIVE backup declares rails', (t) => {
  const root = tmp(t, 'bypass-audit-archive-window-');
...
test('the idempotency scan does not accept a matching entry that lacks the audit payload', (t) => {
  const root = tmp(t, 'bypass-audit-idem-');
  mkdirSync(join(root, '.adlc'), { recursive: true });
  const shared = {
...
test('an entry whose storeHashAfter disagrees with its own storeHash is not a match', (t) => {
  const root = tmp(t, 'bypass-audit-idem-after-');
  mkdirSync(join(root, '.adlc'), { recursive: true });
  const honest = {
...
test('a migration whose apply evidence predates the audit fields can still be recovered', (t) => {
  const root = tmp(t, 'bypass-audit-legacy-evidence-');
  writeLegacy(root, [ticket('T-RAILED', { rails: ['src/**'] })]);
  assert.throws(() => migrateLegacyStore(root, {
    write: true, yes: true, requireClean: false, key: KEY,
    faultInjector(name) { if (name === 'gitignore-updated') throw new Error('fault'); },
...
test('the audit entry survives a canonical round trip: data is JSON-serializable and the signature covers it', (t) => {
  const root = repo(t, [ticket('T-RAILED', { rails: ['src/**'] })]);
