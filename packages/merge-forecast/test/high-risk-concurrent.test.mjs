...
describe('merge-forecast: high-risk (non-veto) concurrent pairs gate-fail (#681)', () => {
  test('AC1 — namespace-collision (0.8) pair in the same wave gate-fails with no --width', async (t) => {
    const root = tmp(t, 'mf-hrc-test-');
  ...
  test('AC2 — a below-threshold pair in the same wave does not gate-fail', async (t) => {
    const root = tmp(t, 'mf-hrc-test-');
    gitInit(root);
  ...
  test('AC3 — a high-risk pair scheduled in DIFFERENT waves does not gate-fail', async (t) => {
    const root = tmp(t, 'mf-hrc-test-');
    namespaceCollisionRepo(root);
  ...
  test('AC4 — regression: an existing hard-veto (score 1.0, scope-overlap) concurrent pair still gate-fails', async (t) => {
    const root = tmp(t, 'mf-hrc-test-');
    gitInit(root);
  ...
  test('AC5 — --conflict-threshold changes which pairs trigger the concurrent gate', async (t) => {
    const root = tmp(t, 'mf-hrc-test-');
    namespaceCollisionRepo(root);
    const tickets = namespaceCollisionTickets();
