...
test('planRound admits only tickets whose predecessors merged (AC3a)', () => {
...
test('planRound never admits completed:true tickets but honors their edges (AC3b)', () => {
...
test('planRound never exceeds the concurrency cap (AC3c)', () => {
...
test('planRound serializes scope-overlapping tickets (AC3d)', () => {
...
test('happy path: build→gate→prosecute→merge → merged in one strike', () => {
  const r = advanceTicket(T('T1'), {
...
test('two-strike: first build fails with dead-end context, second passes (AC3e)', () => {
  const seen = [];
  let n = 0;
  const r = advanceTicket(T('T1'), {
...
test('two-strike: second failure marks the ticket failed (AC3e)', () => {
  const r = advanceTicket(T('T1'), {
...
test('a diagnosed flail skips the second strike (AC3f)', () => {
  let calls = 0;
  const r = advanceTicket(T('T1'), {
...
test('a TICKET-BLOCKED worker marks blocked without consuming the 2nd strike', () => {
  let calls = 0;
  const r = advanceTicket(T('T1'), {
...
test('blocking prosecution routes to a fix strike, merges only after clean re-prosecution (AC3i)', () => {
  let pn = 0;
  const r = advanceTicket(T('T1'), {
...
test('unavailable prosecution fails closed — never merges (F3)', () => {
  let merged = false;
  const r = advanceTicket(T('T1'), {
    dispatch: okBuild, gate: okGate,
...
test('failed post-merge gate triggers revert path and consumes strikes (AC3g)', () => {
  let reverts = 0;
  const r = advanceTicket(T('T1'), {
...
test('resume classifies merged-by-integration-ancestry and does not re-dispatch (AC3h/N2)', () => {
  const all = [T('T1'), T('T2')];
  const status = {
