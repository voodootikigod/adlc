 ...
 * from), never base (adversarial-review N3).
 */
function buildEffects(ticket, wt, deps, integrationBranch) {
  return {
    ...
    merge: () => {
      const { mergeSha, preMergeSha } = deps.mergeToIntegration({ ticket, branch: wt.branch, integrationBranch });
      const post = deps.postMergeGate({ ticket, integrationBranch });
      if (!post.ok) {
        const rev = deps.revertMerge({ integrationBranch, mergeSha, preMergeSha });
 ...
 */
export function runFleet({ all, runId, config, deps }) {
  const integrationBranch = integrationBranchName(runId);
  const log = deps.log ?? (() => {});
  deps.createIntegrationBranch?.({ integrationBranch, baseSha: config.baseSha });
  ...
  // Event-driven loop: recompute the ready set after each batch until nothing
  // more can be admitted (all remaining tickets are terminal or edge-blocked).
  // Merges are sequential (§9): tickets are advanced one at a time here.
  for (;;) {
    const { admit } = planRound(all, { statusById: statusById(status), inFlightIds: inFlightIds(status), cap, onlyIds });
    if (admit.length === 0) break;
    for (const ticket of admit) {
      const wt = deps.createWorktree({ ticket, integrationBranch });
      status = withTicket(status, ticket.id, { state: 'building', branch: wt.branch, startSha: wt.startSha, strikes: 0 });
      persist();
      deps.provision?.({ ticket, worktree: wt.path });
      const outcome = advanceTicket(ticket, buildEffects(ticket, wt, deps, integrationBranch), { log });
      status = withTicket(status, ticket.id, { state: outcome.state, strikes: outcome.strikes, reason: outcome.reason });
      persist();
      deps.cleanup?.({ ticket, worktree: wt.path, state: outcome.state });
