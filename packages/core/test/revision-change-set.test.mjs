...
import { mkdirSync, writeFileSync, rmSync, chmodSync, symlinkSync } from 'node:fs';
import { join } from 'node:path';
import { execFileSync } from 'node:child_process';
import { createHash } from 'node:crypto';
import { resolveChangeSetRevision, changeSetDigest } from '../lib/revision.mjs';
import { gitRepo } from '../lib/test-kit.mjs';

function repo(t) {
  const { dir, g } = gitRepo(t, { prefix: 'adlc-changeset-' });
...
describe('resolveChangeSetRevision (#365) — identity bound to the reviewed change', () => {
  it('is stable when the BASE gains unrelated commits (the whole point)', (t) => {
    const r = repo(t);
    withChange(r);
  ...
  it('changes when a reviewed byte changes', (t) => {
    const r = repo(t);
    withChange(r);
    const before = resolveChangeSetRevision({ cwd: r.dir, base: 'main' });
  ...
  it('F2: identity is independent of diff.algorithm, diff.renames and core.autocrlf', (t) => {
    const r = repo(t);
    withChange(r);
  ...
  it('F2: a rename is recorded as delete+add, not resolved by a similarity heuristic', (t) => {
    const r = repo(t);
  ...
  it('F3: a mode-only change (chmod +x) yields a DIFFERENT identity', (t) => {
    const r = repo(t);
    r.g('checkout', '-q', '-b', 'feat');
  ...
  it('excludes ignorePaths, so recording an attestation does not change the identity', (t) => {
    const r = repo(t);
    r.g('checkout', '-q', '-b', 'feat');
  ...
  it('untracked scratch files do not affect the identity (removes the local-vs-CI divergence)', (t) => {
    const r = repo(t);
    withChange(r);
    const before = resolveChangeSetRevision({ cwd: r.dir, base: 'main' });
  ...
  it('AC13: an UNCOMMITTED edit to a tracked file changes the identity, and reverting restores it', (t) => {
    const r = repo(t);
    withChange(r);
  ...
  it('AC13: two different UNCOMMITTED contents yield different identities (dstsha is zeros for both)', (t) => {
    const r = repo(t);
    withChange(r);
  ...
  it('AC13: an edit made AFTER staging moves the identity (the index sha is not the working tree)', (t) => {
    const r = repo(t);
    withChange(r);
  ...
  it('AC17: a DELETED tracked path resolves via the sentinel instead of crashing', (t) => {
    const r = repo(t);
    withChange(r);
  ...
  it('AC17: the deleted-path sentinel is EXACTLY 40 zeros, not merely "some" stable constant', (t) => {
    const r = repo(t);
  ...
  it('handles a retargeted SYMLINK, including one pointing at a directory', (t) => {
    const r = repo(t);
  ...
  it('AC16: with a text attribute, CRLF content hashes identically under any core.autocrlf', (t) => {
    const r = repo(t);
  ...
  it('carries the gate-derived base sha in the identity, and is prefixed distinctly from the old form', (t) => {
    const r = repo(t);
    withChange(r);
    const rev = resolveChangeSetRevision({ cwd: r.dir, base: 'main' });
