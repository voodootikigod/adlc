...
describe('recordFinding', () => {
  test('appends a finding in the foundry schema, readable via readEntries', (t) => {
    const dir = tmp(t, 'adlc-record-finding-');
  ...
  test('defaults category=prosecution, verdict=open, severity=medium, line=1', (t) => {
    const dir = tmp(t, 'adlc-record-finding-');
  ...
  test('preserves an explicit killed verdict (a refuted finding foundry will filter)', (t) => {
    const dir = tmp(t, 'adlc-record-finding-');
    const entry = recordFinding({ file: 'a/b.mjs', desc: 'refuted', verdict: 'killed' }, dir);
    assert.equal(entry.verdict, 'killed');
  });

  test('appends (does not overwrite) across multiple calls', (t) => {
    const dir = tmp(t, 'adlc-record-finding-');
  ...
  test('throws on a missing/blank file (never silently drops the finding)', (t) => {
    const dir = tmp(t, 'adlc-record-finding-');
  ...
  describe('secret boundary on the committed ledger', () => {
    const reject = (t, desc, re) => {
      const dir = tmp(t, 'adlc-record-finding-');
      assert.throws(() => recordFinding({ file: 'a.mjs', desc }, dir), re);
    };

    test('refuses descriptions carrying credential-shaped values', (t) => {
      reject(t, 'leaked AKIA1234567890ABCDEF in the config', /AWS access key/i);
      reject(t, 'token ghp_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa was committed', /GitHub token/i);
      reject(t, '-----BEGIN RSA PRIVATE KEY----- MIIEowIBAAK', /private key/i);
      reject(t, 'called with Authorization: Bearer abcdefghijklmnopqrstuvwxyz012345', /bearer token/i);
      reject(t, 'config had password: hunter2supersecret', /inline credential/i);
    });

    test('refuses raw multi-line tool output', (t) => {
      reject(t, 'stack trace:\n  at foo (a.mjs:1)\n  at bar (b.mjs:2)', /single line of curated prose/i);
    });

    test('refuses a long hex-encoded secret ANYWHERE in the entry, but exempts sha-length hex citations', (t) => {
      const dir = tmp(t, 'adlc-record-finding-');
    ...
    test('refuses a description long enough to be a dump', (t) => {
      reject(t, 'x'.repeat(601), /capped at/i);
    });

    test('refuses an oversized raw dump smuggled into a NON-desc field', (t) => {
    ...
    test('refuses a many-line dump (log paste / diff hunk / stack trace) in a NON-desc field', (t) => {
      const dir = tmp(t, 'adlc-record-finding-');
    ...
    test('still accepts a finding with a SHORT multi-line evidence quote', (t) => {
    ...
    test('the entry-size cap is enforced at the EXACT boundary (an entry one byte over is rejected)', (t) => {
      const dir = tmp(t, 'adlc-record-finding-');
    ...
    test('refuses a non-finding entry that would crash the P7 pipeline', (t) => {
    ...
    test('still accepts ordinary curated prose describing a failure class', (t) => {
      const dir = tmp(t, 'adlc-record-finding-');
      const entry = recordFinding({
