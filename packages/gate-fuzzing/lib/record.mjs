...
export function writeReproArtifact(defeat, dir = '.') {
  ...
  const id = defeat.id ?? `cand-${Date.now()}`;
  const target = defeat.target;
  // Ensure the computed ID is reflected in the scaffold.
  ...
  const reproPath = join(defeatsDir, `${id}.json`);
  writeFileSync(reproPath, JSON.stringify(artifact, null, 2), 'utf8');

  // Also write the RED test scaffold to the target package's test directory.
  // path: packages/${target}/test/bypass-${id}.test.mjs
  const targetTestDir = join(dir, 'packages', target, 'test');
  try {
    mkdirSync(targetTestDir, { recursive: true });
    const scaffoldPath = join(targetTestDir, `bypass-${id}.test.mjs`);
    writeFileSync(scaffoldPath, scaffold, 'utf8');
  } catch (err) {
    // If we can't write the scaffold (e.g. target dir doesn't exist as expected),
    // we log but don't fail the artifact write.
    console.error(`Warning: could not write test scaffold to ${targetTestDir}: ${err.message}`);
  }
