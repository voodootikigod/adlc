...
`isHardDegraded()`. Thresholds only via `lib/thresholds.mjs` (adapters MUST import; literal-ban CI is a later slice). Band edges are inclusive (`>=`); `@adlc/build-gate` consumes `isHardDegraded()` and uses the same inclusive hard edges (slice-3 adopted).
