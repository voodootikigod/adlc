...
| `@adlc/copilot` | none (Copilot plugin) | Native ADLC plugin for the GitHub Copilot CLI (`plugins/adlc-copilot`): skills, `preToolUse` rails/build-gate hooks, allowlisted MCP gate tools, and read-only prosecution agents. Rail decision delegated to `@adlc/core`; in-session hook is advisory (fail-open host), CI `rails-guard-ci` is the enforcement tier. | [`docs/integrations/copilot.md`](./integrations/copilot.md) |
