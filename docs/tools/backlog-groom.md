...
caller's choosing. The commit it is read at comes from the REMOTE — the forge's
default branch and `ls-remote`, with the merge base required to be reachable from
it — so a local `git update-ref` cannot move the comparison point either, and a
remote that cannot be reached refuses the run instead of falling back. A relabel's `field` is exactly `priority` or `area`, it names
