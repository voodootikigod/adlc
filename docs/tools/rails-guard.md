...
exempted via the CI-plumbing `--sanctioned-add` flag for this run — empty when none were
used. `railsDiffEmpty`'s own meaning is unchanged: it is still `true` whenever there are zero
