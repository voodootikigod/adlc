...
4. **Evaluate sequentially**: for each candidate:
   - Parse and validate JSON. Discard if invalid, if any `file` is not in `--files`, or if `changes` is an empty array — a candidate proposing no changes is a no-op, not a fix, and is never allowed to become a survivor (it would otherwise apply cleanly and be judged solely by whether `--test-cmd` happens to pass a second time against the untouched source).
