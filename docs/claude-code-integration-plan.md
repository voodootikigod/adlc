...
| Rail freeze (P3 / C5) | `rails-guard` | **PreToolUse hook** on `Edit`/`Write`/`MultiEdit` (in-session, precise) + the diff-based `rails-guard` gate in CI (commit-time backstop — template at `docs/ci/rails-guard.yml`, script `scripts/rails-guard-ci.mjs`) | Block structured edits to paths declared `rails` in a ticket + freeze ADLC trust roots. **Bash is not gated in-session** (a shell can't be reliably parsed — see the ADR); rail mutations via Bash are caught by the unbypassable CI diff gate at commit time. The standalone `scripts/rails-guard-ci.mjs` includes base-rail, config-integrity, ticket-contract, and manifest append-only checks that can run without GitHub context; non-GitHub CI must still port the bootstrap acknowledgement, CODEOWNERS self-protection, and signed runner-pool checks from `docs/ci/rails-guard.yml` before treating it as a complete enforcement boundary. |
