...
`@adlc/cli`/plugin lockstep-versioning requirement (1.4.2 or newer is specifically
what makes the `adlc mcp-server` entrypoint available). Codex installs the native
plugin through its Git marketplace; installing `@adlc/codex` with npm alone does
