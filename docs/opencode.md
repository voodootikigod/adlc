...
| `/adlc-prosecute` | P5 | Verifies the build gate (`adlc-runner run p4` (Proposed `@adlc-runner` Extension)). Triggers `adlc review-calibration` (which runs in the background using direct provider environment keys rather than keyless cascade stdio) on model change ([ADLC-C8]). **Bounded run runs prosecute-lite**: 2 lenses (correctness and diff) for exactly 1 pass to limit friction. **Substantial/Architectural runs full prosecution**: 5 lenses fanned out and looping until 2 consecutive passes run dry, up to $R_{\text{max}}$ rounds. Runs `adlc-runner run p5` to assert evidence. |
