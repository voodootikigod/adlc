...
- **Frontier-Free Scaling:** Local models that fail dynamic calibrations are scaled up to N-pass parallel checks (ADLC Exploit E1 sampling diversity) and require majority consensus thresholds ($k$-out-of-$N$), dynamically benchmarked during Phase 5 initialization. Calibration (`review-calibration`) executes locally in the background using direct provider environment keys rather than keyless cascade stdio.
