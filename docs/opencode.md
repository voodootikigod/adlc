...
| --- | --- | --- | --- |
| **preflight** | `session.created` | Advisory | Warns the user if Node/Bun runtimes, git trees, or providers are misconfigured. Reads cached benchmark configuration (runs calibration benchmark only on model changes or if cache is missing). |
| **rails-guard** | `tool.execute.before` | **Enforcing** | Intercepts structured editing tools using a default-intercept approach (gates all tool calls with path parameters, excluding read-only tools). Denies edits to paths declared as `rails` and locks `.adlc/`, `.opencode/`, `.git/`, `.github/`, and `tickets.json` files. Parses `apply_patch` payloads. Traverses path segments to block symlink creation bypasses. Scans diffs for unauthorized test skip/xfail decorators or lint-suppress comments (Goodhart defense). |
| **flail-detection** | `tool.execute.after` | **Enforcing** | (Proposed SDK Extension Specification). Scans outputs for repeated loop errors, scope drift, or excessive logs. Strike 1 (Enforcing, Non-Destructive): builder subagent termination, safe git stash, failed approaches/logs appended to `.adlc/tickets.json` history, and context restart. Strike 2 (Enforcing, Non-Destructive): builder subagent is terminated, staged work is safely stashed, and ticket is escalated back to Phase 2 (Decompose) for ticket partition boundary repair. |
