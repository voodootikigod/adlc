...
| `/adlc-maintain` | C10/C12 | Run decay-driven checks: stale skills, hot files, and gate calibration (`adlc-runner run maintenance`). |
...
| **Maintenance** | `/adlc-maintain` / CI Actions | calibrates reviewers, checks skill-rot and model-ratchet via `adlc-runner run maintenance`. |
