...
| **rails-guard** | `tool.execute.before` | **Enforcing** (Requires SDK support; else advisory + CI backstop) | Intercepts structured editing tools using default-intercept (gates all tools with path parameters, excluding read-only). Denies edits to paths declared as `rails`. During build sessions, pending rails are blocked entirely; pending rails can only be written during a dedicated `/adlc-rail-write` invocation state to defend creator/critic isolation ([ADLC Principle 5]). Once Phase 3 is asserted, rails transition to Frozen (fully blocked). Locks `.adlc/`, `.opencode/`, `.git/`, `.github/`, and `tickets.json` files, parses patches, and scans for unauthorized skips/suppressions (Goodhart defense). |
| **flail-detection** | `tool.execute.after` | **Enforcing** (Requires SDK support; else advisory) | (Proposed SDK Extension Specification). Scans outputs for flailing. Strike 1: terminates builder subagent, stashes uncommitted work safely, appends logs to `.adlc/tickets.json` history, and restarts builder. Strike 2: terminates builder, stashes staged work, and escalates ticket back to Phase 2 (Decompose) for ticket partition repair. |
...
- **Pending vs. Frozen Rails:** To enforce creator/critic isolation in environments lacking native agent-identity verification APIs (preventing builder agents from writing their own tests), the hook **blocks all rail edits during builder sessions entirely**. Pending rails can only be written during a dedicated `/adlc-rail-write` invocation state. Once Phase 3 is complete, rails are marked Frozen and blocked for all agents.
