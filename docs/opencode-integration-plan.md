...
To prevent runtime resolution failures, logic drift, and drifting copies (violating the ADR's companion direction), the plugin does **not** duplicate or manually vendor the core rails-checking, path-resolution, and patch-parsing helper functions. Instead, the plugin declares a dependency on the shared library `@adlc/core` (or `@adlc/hook-core`), and this shared library is **pre-bundled** at build time into the plugin's single-file distribution bundle (`plugin/adlc-opencode/dist/index.js`) using a bundler.

The hook executes with a strict fail-closed contract:
- **Phase Gating & Triage-Class Skips (H1, M1):**
  ...
  - If the active ticket is triaged as **Trivial**, the hook skips P1, P2, and P3 gates entirely (marking them as auto-passed/skipped in the manifest) and permits edits immediately, requiring only a single final prosecution pass (P5) and human acceptance (P6).
...
- **OpenCode Fail-Closed Hook Configuration (H3, H4):**
...
- **Auditable Bypass:** Setting `ADLC_RAILS_BYPASS=1` is only permitted if the environment variable is validated through a secure human-in-the-loop prompt in the TUI (via the proposed plugin UI API). In non-interactive/headless runs (where no TUI is available), the bypass is **refused (fails closed)**.
...
#### Absolute Bin Resolution (M4):
To prevent command-not-found failures and sandbox execution escapes, the plugin does NOT spawn bare command strings (like `adlc`) or rely on a PATH lookup (which is fragile under global installations and subshells, M4). Instead, the plugin dynamically resolves `@adlc/cli` and `@adlc/runner` at runtime using Node's resolution API: `require.resolve('@adlc/cli/package.json')` and `require.resolve('@adlc/runner/package.json')`. It parses their `bin` absolute path mappings and spawns the child processes using Bun's native `Bun.spawn` with these absolute paths.

#### Phase 1: Prompt Extraction
1. **Spawn Extraction Process:** The bridge spawns the absolute CLI path command: `adlc <toolName> <args> --prompt-only`.
...
1. **Spawn Execution Process:** The bridge spawns the absolute CLI path *again* in execution mode (without the `--prompt-only` flag): `adlc <toolName> <args>`.
...
To enable keyless execution for the phase runner (`adlc-runner`), the runner is updated to bubble up fanned-out grandchildren prompts in a structured two-phase manner. (Note: Integrating this cascade protocol requires not only merging `@adlc/runner` from the sibling Codex branch, but also extending its execution loop to implement the correlation ID JSON-over-stdio bubble/inject protocol, M3):
...
- **Frontier-Free Scaling via Dynamic Calibration (L4):**
  ...
  - **Calibration Caching:** To avoid the high computational cost of running review-calibration on every single session start, the plugin caches the calibrated $N$ and $k$ consensus settings in `.adlc/config.json`. The benchmark is only executed during `/adlc-init` or when the developer changes the local model configuration, rather than on every `session.created` preflight check.
