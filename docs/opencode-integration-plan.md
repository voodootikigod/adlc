...
Because `@adlc/*` CLI tools comply with the `--prompt-only` execution contract, they print the generated prompt to `stdout` and **exit immediately with code 0** (rather than holding stdin open or waiting for interaction). This single-shot behavior works perfectly for standard single-prompt gates.
...
### 6.6. Shared Hook-Core Refactor Prerequisite (H1)
* **Status:** ⚠️ Hard Prerequisite (Unimplemented Core Work)
* **Reason:** The proposed shared-hook-core factorization (`@adlc/core` or `@adlc/hook-core` containing common rails-guard logic) is a target ADR design and has not yet been executed in the main codebase. Today, validation logic and helpers (like `globMatch`) are duplicated verbatim in `hooks/adlc-hook.mjs`.
* **Mitigation:** The plugin's Phase D implementation is blocked until the shared core package is factored and published. If integration is forced today, the plugin must vendor the path resolution and validation logic directly, maintaining a strict `"KEEP IN SYNC"` contract with the parent repository's hooks to prevent logic drift.
