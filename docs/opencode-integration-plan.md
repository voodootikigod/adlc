...
- **Frontier-Free Scaling via Dynamic Calibration (L4):**
  ...
  - **Calibration Caching (M2):** To avoid the high computational cost of running review-calibration on every single session start, the plugin caches the calibrated stack size $N$ and consensus $k$ settings in `.adlc/config.json`. The preflight hook (`session.created`) check resolves first: if the `.adlc/` directory does not exist (uninitialized repository), the preflight hook immediately returns a **no-op** to prevent running benchmarks on unrelated clean workspaces (L1). If initialized, the hook reads this cached configuration and only prompts for a recalibration run on model changes or if the cache is missing.
