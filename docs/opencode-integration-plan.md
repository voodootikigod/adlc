...
- **Single-Shot Fallback:** For tools lacking `--prompt-session` support, the bridge falls back to standard `--prompt-only` mode, extracting and answering prompts sequentially in independent, single-shot CLI invocations.
