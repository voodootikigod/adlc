...
|---|---|---|---|
| **Preflight** (SessionStart Check) | `adlc preflight` | `session.created` / `session.start` | **Advisory**: Checks system requirements, Node/Bun runtimes, and git state. Logs warnings but never blocks the session (asserting environment determinism, ADLC Appendix F / D2). |
...
| **Build Gate** (P4) | `adlc-runner run p4` (assertion) | **Slash Command** (`/adlc-prosecute` execution boundary) | **Enforcing**: Asserts that the deterministic build gate is satisfied (verifying that recorded build logs show successful compilation, lints are clean, and all rail tests pass on the current implementation as recorded in `.adlc/` by builder/slash commands) prior to beginning Phase 5 prosecution. Blocks the transition if failures exist (M3). |
