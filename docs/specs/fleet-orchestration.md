...
| `--model-plane-read bounded` + `--model-plane-read-only <abs,…>` | the MODEL plane uses the bounded read policy (worktree + synthetic home + allowlist); the fixed system roots the host has (`/usr`, `/lib`, `/lib64`, `/etc/ssl`, `/etc/resolv.conf`, `/etc/hosts`) are always bound and the flag extends them with a private empty tmpfs `/tmp` (`TMPDIR` inside it); a FILE entry is a single-file bind; requires bubblewrap (Seatbelt cannot express it — fail closed) |
