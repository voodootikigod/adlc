...
- Edit the **adlc phase router**, which is replicated across **five harness plugins** with
  three different file conventions — **all five must be edited** or discoverability is
  inconsistent across harnesses:
  - `plugins/adlc-claude-code/skills/adlc/SKILL.md`
  - `plugins/adlc-codex/skills/adlc/SKILL.md`
  - `plugins/adlc-pi/skills/adlc/SKILL.md`
  - `plugins/adlc-opencode/skill/adlc.md` (note: singular `skill/`, lowercase `adlc.md`)
  - `plugins/adlc-cursor/rules/adlc.mdc` (Cursor `.mdc` rules format, table-shaped router)

  In each: reference `adversarial-review` at P1 (design), P3 (rail selection), and P5
  (built code, multi-model), and add a short **"adversarial-review loop"** note:
  cross-model + fresh-context + loop-until-SHIP; `--verify`, `--loop`, `--providers`;
  **exit 0 = SHIP**. (Adapt phrasing to each router's existing shape — prose list vs.
  Cursor's table.)
...
2. **Edits to the adlc phase router in all five harness plugins** — `adlc-claude-code`,
   `adlc-codex`, `adlc-pi` (`skills/adlc/SKILL.md`), `adlc-opencode` (`skill/adlc.md`), and
   `adlc-cursor` (`rules/adlc.mdc`) — insertion point 2.
...
- **AC3** — *Verify:* `grep -lE 'adversarial-review' plugins/adlc-claude-code/skills/adlc/SKILL.md plugins/adlc-codex/skills/adlc/SKILL.md plugins/adlc-pi/skills/adlc/SKILL.md plugins/adlc-opencode/skill/adlc.md plugins/adlc-cursor/rules/adlc.mdc` returns all **five** harness routers, each naming `adversarial-review` (under P1/P3/P5) with an `exit 0 = SHIP` note.
