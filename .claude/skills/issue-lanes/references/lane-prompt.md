...
4. P5 gates (never pipe a gate to tail in a background task; redirect to a scratchpad file and capture the exit code on its own line):
   ...
   b. `node scripts/run-tests.mjs <pkg>` (the root runner's segment) → green.
